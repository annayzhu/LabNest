...
- 应用版本：`979f1b07968057716a85563ff5e6c42f1e82045c`（转染代码 `f619e80`，已合并 main `3f90adb`）。
- 通过：上海/温哥华各576项测试、TypeScript、生产构建、全仓lint（0错误、11条既有警告）；见相应日志。
...
- 并行运行浏览器与全仓测试时，既有Visualization聚类耗时3.16秒超过其3秒门槛；保留在shanghai-concurrent-failed.log，未修改算法或放宽门槛。已停止并发负载后重跑。最终无未解决的本地验收失败；开发时的测试断言/辅助脚本问题已修正，最终结果见 `browser.json`。远端CI以PR的实际状态为准。

## 最终整套浏览器回归

应用代码仍为979f1b0；同步main后的桌面Run也挂载完整内容，手机测试现限定可见步骤。旧复制/最近使用断言按用户的新规则更新，未放宽页面警告、步骤归属或导出追溯检查。最终22个套件全部通过，见 ../acceptance-run.json：首轮20个通过，移液旧复制断言及其依赖DB检查在修正后分别重跑通过；首次失败记录保留在 regression-first-pass.json。覆盖合成旧Run/旧快照、参数化表格、图片、完整离线往返、同步幂等及数据库回读。PDF两页均回读并视觉检查，其他正式格式逐文件比较。远端CI仍以PR实际运行状态为准。
...
- 原生系统打印对话框未执行；PDF采用Chromium生产页面打印，已回读文本并逐页检查。
