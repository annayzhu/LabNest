...
本轮补齐应用代码版本：`7bc8b4c`（完整 SHA 见 Git 提交）。此前 d5ba793 的双时区与远端 CI 是历史证据，不代表本轮新增代码已被这些检查覆盖。最终联合版本另行验证。
...
隔离生产服务3233完整 verify-inventory-all 退出0，包含复制、已保存计时器重置、预计/实际用量、瓶余量关联、学校税额校验/快照和六种选择范围导出。有效长编号 EXP-3000000000 曾导致 PostgreSQL Int 溢出，计数器改为 BigInt 后复制与幂等复跑通过。仅扩充编号容量，不改历史编号；六项新增迁移仅在合成库执行。旧库迁移回读也重新通过。生产构建、TypeScript 日志已保存。最终双时区和远端 CI 以联合交付记录为准。
