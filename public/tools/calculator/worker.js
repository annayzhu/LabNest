...
self.addEventListener('message',event=>{const type=event.data?.type;if(!['CACHE_CALCULATOR','CALCULATOR_STATUS','CLEAR_CALCULATOR'].includes(type))return;event.waitUntil(preparations=preparations.then(async()=>{let staging,stageTimer;try{
 ...
 }catch(error){if(staging)try{await caches.delete(staging);}catch{/* Failed cleanup must not suppress the reply or poison subsequent retries. */}event.ports[0]?.postMessage({ok:false,kind:error.name==='QuotaExceededError'?'quota':error.name==='AbortError'?'timeout':'resource',detail:String(error)});}finally{clearTimeout(stageTimer);}}));});
