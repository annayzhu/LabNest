/* Explicit offline cache for Calculator and immutable Run page snapshots; never API writes. */
const cacheName = 'labnest-calculator-shell-v4';
...
self.addEventListener('message', event => {
  ...
  event.waitUntil((async () => {
    ...
    const urls = [...new Set(event.data.urls)].filter(value => {
      try { const url = new URL(value);return url.origin === self.location.origin && (url.pathname.startsWith('/tools/calculator') || /^\/experiments\/[^/]+\/run$/.test(url.pathname) || url.pathname.startsWith('/_next/static/') || url.pathname.startsWith('/icons/lab-soft-v1/')); } catch { return false; }
    });
    try {
...
self.addEventListener('fetch', event => {
  ...
  if (event.request.method !== 'GET' || url.origin !== self.location.origin || !(url.pathname.startsWith('/tools/calculator') || /^\/experiments\/[^/]+\/run$/.test(url.pathname) || url.pathname.startsWith('/_next/static/') || url.pathname.startsWith('/icons/lab-soft-v1/'))) return;
