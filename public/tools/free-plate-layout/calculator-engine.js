  ...
  function fourPlFit(rows) {
    ...
    for (let iteration = 0; iteration < 800; iteration += 1) {
      ...
      for (let index = 0; index < 4; index += 1) for (const direction of [-1, 1]) {
        ...
        if (candidate[1] <= candidate[0] || candidate[2] <= 0 || Math.abs(candidate[3]) <= 0.05 || Math.abs(candidate[3]) > 10) continue;
