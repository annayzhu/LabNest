...
const inventory=await prisma.inventoryItem.create({data:{name:'Synthetic debug buffer',currentQuantity:100,unit:'mL'}});
writeFileSync('docs/debug-20260909/evidence/run-fixture.json',JSON.stringify({experimentId:exp.id,inventoryItemId:inventory.id}));
...
writeFileSync('docs/debug-20260909/evidence/editor/fixtures.json',JSON.stringify({synthetic:true,protocolTitle:protocol.title,projectId:project.id,planId:plan.id,experimentId:exp.id,protocolId:protocol.id,versionId:version.id,cases},null,2));
}
