...
assert(new URL(process.env.DATABASE_URL!).pathname === '/labnest_stage_a_acceptance');
async function main(){
...
writeFileSync('docs/debug-20260909/evidence/font-fixtures.json',JSON.stringify({synthetic:true,protocolTitle:protocol.title,projectId:project.id,planId:plan.id,experimentId:exp.id,protocolId:protocol.id,versionId:version.id,cases},null,2));
}
