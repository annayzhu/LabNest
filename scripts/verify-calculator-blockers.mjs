...
try{
 ...
 await p.getByRole('button',{name:'Calculate',exact:true}).click();await showCalculatorResult(p);
 ...
 const downloading=p.waitForEvent('download');await openCalculatorDisclosure(p,'Export');await p.getByRole('button',{name:'Export CSV',exact:true}).click();const download=await downloading;await download.saveAs(dir+'/gradient.csv');assert(!(await readFile(dir+'/gradient.csv','utf8')).includes('tube:1'));
