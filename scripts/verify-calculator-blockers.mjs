...
try{
 ...
 const downloading=p.waitForEvent('download');await p.getByRole('button',{name:'CSV',exact:true}).click();const download=await downloading;await download.saveAs(dir+'/gradient.csv');assert(!(await readFile(dir+'/gradient.csv','utf8')).includes('tube:1'));
