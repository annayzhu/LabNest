...
try{for(const width of [1440,390])for(const mode of ['light','dark']){
 ...
 const calculate=p.getByRole('button',{name:'Calculate',exact:true});const calcBox=await calculate.boundingBox(),presetBox=await save.boundingBox();assert(presetBox.y>=calcBox.y+calcBox.height);
 ...
 await p.getByText('Choose / manage presets',{exact:true}).click();await p.getByRole('textbox',{name:'Search presets',exact:true}).fill('preset 13');await p.getByRole('textbox',{name:'Search presets',exact:true}).press('Enter');assert.equal(await p.locator('[role=alert]').count(),0);assert.equal(await p.locator('.calculator-preset-list > div').count(),1);
 ...
 await p.evaluate(()=>{const k='labnest.calculators.v1',s=JSON.parse(localStorage.getItem(k));s.presets=[{id:'mass',calculatorId:'unit-converter',name:'Mass reference',createdAt:'2026-09-15',methodVersion:'unit-converter-v1',inputs:{dimension:'mass',value:2,fromUnit:'mg',toUnit:'µg'}}];localStorage.setItem(k,JSON.stringify(s));});await p.reload({waitUntil:'networkidle'});await p.getByRole('combobox',{name:'Presets',exact:true}).selectOption('mass');await p.getByRole('button',{name:'Load',exact:true}).click();assert.equal(await p.getByRole('textbox',{name:'From unit',exact:true}).inputValue(),'mg');assert.equal(await p.getByRole('textbox',{name:'To unit',exact:true}).inputValue(),'µg');assert((await p.locator('[data-calculator-result]').innerText()).includes('2000'));checks.push({status:'passed',check:'cross-dimension preset retains mg to µg and computes 2000'});await c.close();
