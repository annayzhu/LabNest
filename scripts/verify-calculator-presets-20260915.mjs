...
try{for(const width of [1440,390])for(const mode of ['light','dark']){
 ...
 await p.getByText('Choose / manage presets',{exact:true}).click();await p.getByRole('textbox',{name:'Search presets',exact:true}).fill('preset 13');assert.equal(await p.locator('.calculator-preset-list > div').count(),1);
