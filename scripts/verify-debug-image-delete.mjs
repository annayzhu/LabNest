...
const f=JSON.parse(readFileSync('docs/editor-repair/evidence/fixtures.json'));const browser=await chromium.launch();const page=await browser.newPage();const records=[];
try{
 ...
 const media=page.locator('[data-document-media]').first();await media.getByRole('img').waitFor();await media.getByRole('img').click();await page.getByRole('button',{name:'移除引用 / Remove reference',exact:true}).click();
