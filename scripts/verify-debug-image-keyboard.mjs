...
try{
 ...
 await page.goto(base+item.edit);const body=page.locator('[contenteditable="true"]').first();await body.click();await page.keyboard.press('End');await page.keyboard.press('Enter');
 ...
 const inserted=body.locator('[data-document-media] img[alt="delete-fixture.png"]');await inserted.waitFor();const mediaId=await inserted.locator('xpath=ancestor::*[@data-document-media][1]').getAttribute('data-document-media');const media=page.locator(`[data-document-media="${mediaId}"]`);await media.getByRole('img').click();await media.focus();await page.keyboard.press('Backspace');
 ...
 writeFileSync('docs/debug-20260909/evidence/image-keyboard.json',JSON.stringify(records,null,2));
