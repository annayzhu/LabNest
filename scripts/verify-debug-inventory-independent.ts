...
async function main() {
  ...
  const base = process.env.LABNEST_ACCEPTANCE_URL ?? 'http://localhost:3232';
  if (base !== 'http://localhost:3232') throw new Error('Isolated production acceptance server only');
  const f = JSON.parse(readFileSync('docs/debug-20260909/evidence/material-boundary-fixture.json', 'utf8'));
  ...
  try {
    ...
    writeFileSync('docs/debug-20260909/evidence/inventory-independent.json', JSON.stringify({ fixture: 'Synthetic Run creation via Prisma; completion through real mobile page', checks: ['Run with no purchases/inventory completes', 'Run with pending insufficient consumption completes without automatic deduction or losing saved material rows'] }, null, 2));
