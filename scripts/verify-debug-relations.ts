...
async function main(){
 ...
 const base=process.env.LABNEST_ACCEPTANCE_URL??'http://localhost:3232';if(!['http://localhost:3232','http://localhost:3235'].includes(base))throw Error('Isolated server only');
 ...
 try{
  ...
  writeFileSync('docs/debug-20260909/evidence/shared-relations.json',JSON.stringify({planId:plan.id,checks},null,2));
