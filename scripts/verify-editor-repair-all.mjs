...
function run(name,args,env={}){const fd=openSync(`${dir}/${name}.log`,'w');const result=spawnSync(process.execPath,args,{env:{...process.env,...env},stdio:['ignore',fd,fd]});closeSync(fd);report.checks.push({name,status:result.status===0?'passed':'failed',exitCode:result.status});writeFileSync(`${dir}/acceptance.json`,JSON.stringify(report,null,2));console.log(`${name}: ${result.status===0?'passed':'failed'}`);return result.status===0;}
