...
if(phase==='run'){
 const protocol=fixtures.cases.find(c=>c.name==='protocol');await page.goto(base+protocol.edit,{waitUntil:'networkidle'});const caption='Run step image '+Date.now();const media=await insertImage(page,'run-step.png',page.locator('[data-section-key="steps"] .ln-protocol-section-content p').filter({hasText:'Editor acceptance body'}).first());await page.getByRole('textbox',{name:'图注 / Caption',exact:true}).fill(caption);if(await page.getByRole('button',{name:'收起属性',exact:true}).isVisible())await page.getByRole('button',{name:'收起属性',exact:true}).click();await page.getByRole('button',{name:'Save Protocol',exact:true}).click();await page.waitForURL(url=>!url.pathname.endsWith('/edit'));
