...
const base=process.env.LABNEST_EDITOR_TEST_URL||'http://localhost:3227';assert(['3227','3234','3235'].includes(new URL(base).port)&&['localhost','127.0.0.1'].includes(new URL(base).hostname),'Use isolated editor acceptance server');
...
if(phase==='ime'){
 ...
 const media=await insertImage(page,`ime-${item.name}.png`);const caption=page.getByRole('textbox',{name:'图注 / Caption',exact:true});await caption.fill('实验 image');
...
if(phase==='media-controls'){
 ...
 assert.equal(await page.getByRole('textbox',{name:'图注 / Caption',exact:true}).isVisible(),false,'Unselected image has no permanent property panel');
 ...
 await page.getByRole('textbox',{name:'Experiment title',exact:true}).click();await media.focus();assert(await page.getByRole('textbox',{name:'图注 / Caption',exact:true}).isVisible());
 ...
 if(await page.getByRole('button',{name:'收起属性',exact:true}).isVisible())await page.getByRole('button',{name:'收起属性',exact:true}).click();await page.getByRole('button',{name:item.save,exact:true}).click();await page.waitForURL(base+item.view);await page.reload({waitUntil:'networkidle'});const replaced=page.getByRole('img',{name:'中文 image 说明',exact:true});await replaced.evaluate(i=>i.decode());assert.notEqual(await replaced.getAttribute('src'),originalSrc);
 report.checks.push('Selection, optional width, keyboard focus/expand and replacement save/refresh passed');
}
if(phase==='metadata'){
 for(const width of [1440,390]){await page.setViewportSize({width,height:1000});await page.getByRole('tab',{name:'Metadata',exact:true}).click();const panel=page.locator('.context-properties:not([hidden])');await panel.waitFor();await panel.evaluate(el=>Promise.all(el.getAnimations().map(a=>a.finished)));const geometry=await panel.evaluate(el=>{const r=el.getBoundingClientRect();return {left:r.left,right:r.right,width:r.width,viewport:innerWidth};});assert(Math.abs(geometry.right-width)<2,JSON.stringify(geometry));assert(geometry.width<=width);assert.equal(await page.locator('.context-properties:not([hidden])').count(),1);await page.getByRole('button',{name:'收起属性',exact:true}).click();report.checks.push({width,geometry,sharedRightPanel:true});}
}
if(phase==='roundtrip'){
 ...
 const media=await insertImage(page,`${item.name}-${Date.now()}.png`);await page.getByRole('textbox',{name:'图注 / Caption',exact:true}).fill(caption);await media.getByText('图片设置 / Image settings',{exact:true}).click();await page.getByRole('spinbutton',{name:'图片显示宽度百分比 / Image width percent',exact:true}).fill('60');
 if(await page.getByRole('button',{name:'收起属性',exact:true}).isVisible())await page.getByRole('button',{name:'收起属性',exact:true}).click();await page.getByRole('button',{name:item.save,exact:true}).click();await page.waitForURL(url=>!url.pathname.endsWith('/edit'));await page.goto(base+item.view,{waitUntil:'networkidle'});await page.reload({waitUntil:'networkidle'});
 ...
 await page.emulateMedia({media:'screen'});await page.goto(base+item.edit,{waitUntil:'networkidle'});await picture.evaluate(i=>i.decode());await picture.click();const node=page.locator('[data-document-media]').filter({has:picture}).first();assert.equal(await page.getByRole('textbox',{name:'图注 / Caption',exact:true}).inputValue(),caption);await node.getByText('图片设置 / Image settings',{exact:true}).click();assert.equal(await page.getByRole('spinbutton',{name:'图片显示宽度百分比 / Image width percent',exact:true}).inputValue(),'60');
...
if(phase==='run'){
 const protocol=fixtures.cases.find(c=>c.name==='protocol');await page.goto(base+protocol.edit,{waitUntil:'networkidle'});const caption='Run step image '+Date.now();const media=await insertImage(page,'run-step.png',page.locator('[data-section-key="steps"] .ln-protocol-section-content p').last());await page.getByRole('textbox',{name:'图注 / Caption',exact:true}).fill(caption);if(await page.getByRole('button',{name:'收起属性',exact:true}).isVisible())await page.getByRole('button',{name:'收起属性',exact:true}).click();await page.getByRole('button',{name:'Save Protocol',exact:true}).click();await page.waitForURL(url=>!url.pathname.endsWith('/edit'));
 await page.goto(base+'/experiments/new',{waitUntil:'networkidle'});await page.getByRole('tab',{name:'Metadata',exact:true}).click();await page.getByRole('button').filter({hasText:fixtures.protocolTitle}).click();await page.locator('[name=title]').fill('Synthetic Run image acceptance');if(await page.getByRole('button',{name:'收起属性',exact:true}).isVisible())await page.getByRole('button',{name:'收起属性',exact:true}).click();await page.getByRole('button',{name:'Save Experiment',exact:true}).click();await page.waitForURL(/\/experiments\/(?!new)/);const url=page.url()+'/run';await page.goto(url,{waitUntil:'networkidle'});
