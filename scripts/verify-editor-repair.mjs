...
if(phase==='media-controls'){
 ...
 assert.equal(await media.getByRole('spinbutton').inputValue(),'60');report.checks.push('Caption on selection; width and replace in optional settings');
}
if(phase==='metadata'){
...
if(phase==='roundtrip'){
 ...
 await page.emulateMedia({media:'print'});await picture.waitFor();const pdf=await page.pdf({path:`${dir}/${item.name}-view.pdf`,format:'A4',printBackground:true});assert(pdf.toString('latin1').includes('/Subtype /Image'),'PDF embeds actual image');
 ...
 await page.emulateMedia({media:'print'});await picture.waitFor();assert((await page.locator('.document-print-root').first().innerText()).includes(caption));const editPdf=await page.pdf({path:`${dir}/${item.name}-edit.pdf`,format:'A4',printBackground:true});assert(editPdf.toString('latin1').includes('/Subtype /Image'));
...
if(phase==='template'){
 ...
 await page.getByRole('button',{name:'Save Protocol',exact:true}).click();await page.waitForURL(url=>!url.pathname.endsWith('/edit'));await page.reload({waitUntil:'networkidle'});const picture=page.getByRole('img',{name:caption,exact:true}).first();await picture.evaluate(i=>i.decode());assert.equal(await page.locator('figure').filter({hasText:'填写说明 / Instructions'}).getByRole('img',{name:caption,exact:true}).count(),1,'Saved image remains in the instruction section');await page.emulateMedia({media:'print'});await picture.waitFor();const pdf=await page.pdf({path:dir+'/template-view.pdf',format:'A4',printBackground:true});assert(pdf.toString('latin1').includes('/Subtype /Image'));
 await page.emulateMedia({media:'screen'});await page.goto(base+item.edit,{waitUntil:'networkidle'});await page.locator('.ln-protocol-result-template-editor').last().locator('summary').first().click();assert.equal(await page.locator('.ln-protocol-result-template-editor').last().getByRole('img',{name:caption,exact:true}).count(),1);await picture.evaluate(i=>i.decode());await page.emulateMedia({media:'print'});await picture.waitFor();const editPdf=await page.pdf({path:dir+'/template-edit.pdf',format:'A4',printBackground:true});assert(editPdf.toString('latin1').includes('/Subtype /Image'));report.checks.push({entry:'result-template-instructions',caption,saveRefreshReadEditPrint:'passed'});
}
if(phase==='run'){
...
}catch(error){report.error=String(error);report.clientErrors=clientErrors;report.pageText=(await page.locator('body').innerText()).slice(0,2500);throw error;}finally{writeFileSync(dir+'/'+phase+'.json',JSON.stringify(report,null,2));await browser.close();}
