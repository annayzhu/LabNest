...
try {
  ...
  for (const module of ["research-plans", "protocols", "experiments", "results", "reports", "entries"]) {
    await page.goto(`${baseUrl}/${module}`, { waitUntil: "networkidle" });
    const href = await page.locator(`a[href^="/${module}/"]`).evaluateAll((links, module) => links.map((link) => link.getAttribute("href")).find((href) => new RegExp(`^/${module}/[^/?]+$`).test(href) && !/\/(new|import|export)$/.test(href)), module);
    assert(href, `${module}: no saved record available to verify`);
    records[module] = href;
    await page.goto(`${baseUrl}${href}`, { waitUntil: "networkidle" });
    assert.equal(await page.locator(".document-canvas-toolbar").count(), 0, `${module}: legacy preview strip remains`);
    assert.equal(await page.locator(".document-editor-sidebar").count(), 0, `${module}: preview still uses editor drawer`);
    ...
    assert(paperBox && paperBox.width > 300, `${module}: paper is not visibly laid out`);
    assert(metadataBox && metadataBox.x >= paperBox.x + paperBox.width - 2, `${module}: metadata is not to the right of the paper`);
    await page.screenshot({ path: `${output}/${module}-preview.png`, fullPage: false });
  }
  ...
  for (const route of ["/protocols/new", "/research-plans/new", "/experiments/new", "/results/new?manual=1", `${records.reports}/edit`, "/entries/new"]) {
    ...
    const print = page.locator(".standalone-document-editor-toolbar").getByRole("button", { name: "Print", exact: true });
    ...
    const tabs = page.locator(".document-editor-layout-tabs");
    if (await tabs.count()) {
      await page.evaluate(() => window.scrollTo(0, 400));
      const tabBox = await tabs.boundingBox();
      const chromeBox = await page.locator(".standalone-document-editor-toolbar").boundingBox();
