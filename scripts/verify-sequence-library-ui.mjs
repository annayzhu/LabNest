...
async function verify(viewport, name) {
  ...
  const importTemplateResponse = await page.request.get(`${baseUrl}/api/sequences/template?format=csv`);
  const importTemplate = await importTemplateResponse.text();
  if (!importTemplate.split("\n")[0].includes("application,transcriptAccession,ampliconLengthBp,exonJunction,targetRegion,designSource")) throw new Error(`${name}: paired metadata columns are missing from the import template`);
  if (!importTemplate.includes("qPCR,NM_000138.5,120,Spans exon junction,,Primer-BLAST")) throw new Error(`${name}: primer-pair metadata example is missing from the import template`);
  ...
  const firstPairName = (await pairLinks.first().innerText()).trim();
  const searchResponse = await page.request.get(`${baseUrl}/api/search?q=${encodeURIComponent(firstPairName)}&limit=20`);
  const searchPayload = await searchResponse.json();
  const exactPairResults = searchPayload.results.filter((result) => result.title === firstPairName);
  if (!exactPairResults.some((result) => result.href === firstPairHref)) throw new Error(`${name}: paired entry is missing from global search`);
  if (exactPairResults.some((result) => /^\/sequences\/[^/]+$/.test(result.href))) throw new Error(`${name}: hidden pair members leak into global search`);
  ...
  if (!await page.locator('textarea[placeholder="GCUACU…TT"]').count()) throw new Error(`${name}: siRNA sequence example is not directly submittable`);
