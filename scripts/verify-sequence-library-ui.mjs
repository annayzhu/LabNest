...
async function verify(viewport, name) {
  ...
  await page.goto(`${baseUrl}/sequences/new?category=oligo`, { waitUntil: "networkidle" });
  if (!await page.locator('input[placeholder="FBN2 capture oligo 1"]').count()) throw new Error(`${name}: oligo-specific name prompt is missing`);
  const oligoTypes = await page.locator('select[name="designType"] option').evaluateAll((options) => options.map((option) => option.value));
  if (JSON.stringify(oligoTypes) !== JSON.stringify(["probe", "shRNA", "gRNA", "oligo", "other"])) throw new Error(`${name}: Oligo design types are not properly scoped`);
