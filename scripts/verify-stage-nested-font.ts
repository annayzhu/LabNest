...
async function main(){
 ...
 try{
  await page.goto(base+`/protocols/${protocol.id}/versions/${version.id}/edit`);await page.getByText('Outer body',{exact:true}).click();await page.getByRole('button',{name:'Font size',exact:true}).click();await page.getByLabel('字号作用范围 / Font size scope').selectOption('document');await page.getByRole('menuitem',{name:'12 pt',exact:true}).click();
  const template=page.locator('.ln-protocol-result-template-editor');await template.locator('summary').first().click();assert.equal(await template.getByText('Nested instruction 42',{exact:true}).evaluate(e=>getComputedStyle(e).fontSize),'16px');
  if(await page.getByRole('button',{name:'收起属性',exact:true}).isVisible())await page.getByRole('button',{name:'收起属性',exact:true}).click();
  await template.getByText('Nested instruction 42',{exact:true}).click();await page.getByRole('button',{name:'Font size',exact:true}).click();await page.getByLabel('字号作用范围 / Font size scope').selectOption('document');await page.getByRole('menuitem',{name:'18 pt',exact:true}).click();assert.equal(await page.getByText('Outer body',{exact:true}).evaluate(e=>getComputedStyle(e).fontSize),'24px');
  if(await page.getByRole('button',{name:'收起属性',exact:true}).isVisible())await page.getByRole('button',{name:'收起属性',exact:true}).click();
