...
export async function POST(request:Request) {
  ...
  try {
    ...
    if(!isDeepStrictEqual(JSON.parse(JSON.stringify(computed)),input.snapshot))return Response.json({error:'方法、单位、警告或结果不一致，请重新确认 / Method, units, warnings or snapshot mismatch; review before saving'}, {status:409});
