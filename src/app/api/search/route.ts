...
export async function GET(request: Request) {
    ...
    prisma.inventoryItem.findMany({
      where: {
        OR: [
          ...
          { principalInvestigator: textFilter(query) },
  ...
  const merged: DatabaseSearchResult[] = [
    ...
    ...inventoryItems.map((item) => ({
      ...
      title: item.name,
      subtitle: item.principalInvestigator ?? item.englishName ?? item.brand ?? item.aliquotCode ?? item.lotNumber ?? item.location?.name,
      href: `/inventory/${item.id}`,
      matchedText: item.principalInvestigator ?? item.casNumber ?? item.catalogNumber ?? item.notes ?? item.storageCondition ?? undefined,
    })),
    ...results.map((result) => ({
      id: result.id,
