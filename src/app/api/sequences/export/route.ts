...
export async function GET(request: Request) {
  ...
  const filteredRecords = versionScope === "latest" ? records.filter((record) => {
  ...
  if (format === "fasta") {
    const body = [...filteredRecords.flatMap((record) => record.versions.map((version) => toFasta(`${record.code}|${record.name.replaceAll(" ", "_")}|v${version.displayVersion}|${version.moleculeType}`, version.sequence))), ...pairs.flatMap((pair) => pair.members.map((member) => toFasta(`${pair.code}|${pair.name.replaceAll(" ", "_")}|${member.role}|v${member.sequenceVersion.displayVersion}|${member.sequenceVersion.moleculeType}`, member.sequenceVersion.sequence)))].join("\n");
