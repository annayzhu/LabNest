...
export default async function ExperimentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const recordFields = [
    ...
    ["Materials / samples used", experiment.materialsText],
    ["Actual procedure notes", experiment.stepsText],
    ["Observations", experiment.observations],
    ["Deviations", experiment.deviations],
    ["Result summary", experiment.resultSummary],
    ["Conclusion", experiment.conclusion],
  ].filter((item): item is [string, string] => Boolean(item[1]));
  return <AppShell><div className="space-y-6">
    <PageHeader eyebrow={`${experiment.researchPlan?.code ?? "Unassigned plan"} · ${experiment.runCode ?? experiment.date.toLocaleDateString()}`} title={experiment.title} description={experiment.purpose ?? "Purpose not recorded."} actions={<><Link href={`/results/new?experiment=${experiment.id}`} className={primaryButton}>Add Result</Link><Link href={`/experiments/${experiment.id}/edit`} className={secondaryButton}>Edit experiment</Link></>} />
    ...
    {recordFields.length ? <Card><CardHeader title="Experiment record" eyebrow="Only populated fields are expanded" /><CardBody className="grid gap-5 md:grid-cols-2">{recordFields.map(([label, value]) => <section key={label} className="border-l border-hairline pl-3"><h3 className="text-xs font-semibold uppercase tracking-[0.08em] text-muted">{label}</h3><p className="mt-2 whitespace-pre-wrap text-sm leading-7 text-graphite">{value}</p></section>)}</CardBody></Card> : null}
