...
import { buildExperimentStepGroupTitle, experimentStepGroupHeading } from "@/lib/experiment-planning";
...
export default async function ExperimentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <AppShell><div className="space-y-6">
    ...
    <div className="document-editor-layout">
      ...
      <aside className="document-editor-sidebar" aria-label="Experiment controls and result recording">
        <Card><CardHeader title="Execution control" eyebrow="Plan and exact method provenance" action={<div className="flex gap-2"><StatusPill status={experiment.status} /><StatusPill status={experiment.recordStatus} /></div>} /><CardBody className="space-y-4">
          ...
          <div className="flex flex-wrap gap-y-4 border-t border-hairline pt-4"><div className="w-1/2 min-w-0"><Control label="Date">{experiment.date.toLocaleDateString()}</Control></div><div className="w-1/2 min-w-0"><Control label="Steps">{completed}/{experiment.steps.length}</Control></div><div className="w-1/2 min-w-0"><Control label="Results">{experiment.results.length}</Control></div><div className="w-1/2 min-w-0"><Control label="Attachments">{attachmentLinks.length}</Control></div></div>
          <div className="flex flex-wrap gap-2 border-t border-hairline pt-3">{experiment.tags.map((tag) => <Badge key={tag}>{tag}</Badge>)}{experiment.primaryProtocolVersion ? <Link href={`/protocols/${experiment.primaryProtocolVersion.protocolId}?version=${experiment.primaryProtocolVersion.id}`} className="text-xs font-medium text-moss hover:underline">Primary: {experiment.primaryProtocolVersion.protocol.humanCode ?? experiment.primaryProtocolVersion.protocol.title} · v{experiment.primaryProtocolVersion.displayVersion}</Link> : null}</div>
        </CardBody></Card>

        <ExperimentResultRecordingCard experimentId={experiment.id} recording={resultRecording} />

        <Card><CardHeader title="Locked ProtocolVersions" eyebrow="Snapshot retained independently of future Protocol edits" /><CardBody><ul className="divide-y divide-hairline">{experiment.protocolVersions.map((row) => { const heading = experimentStepGroupHeading(buildExperimentStepGroupTitle({ humanCode: row.protocolVersion.protocol.humanCode ?? "Uncoded", protocolTitle: row.protocolVersion.protocol.canonicalTitle ?? row.protocolVersion.protocol.title, versionTitle: row.protocolVersion.title, displayVersion: row.protocolVersion.displayVersion })); return <li key={row.protocolVersionId} className="space-y-2 py-3 first:pt-0 last:pb-0"><div className="flex flex-wrap items-start justify-between gap-2"><Link href={`/protocols/${row.protocolVersion.protocolId}?version=${row.protocolVersionId}`} className="min-w-0 break-words text-sm font-semibold text-moss hover:underline">{heading.title}</Link><Badge tone={row.role === "primary" ? "sage" : "neutral"}>{row.role}</Badge></div>{heading.detail ? <p className="text-xs leading-5 text-muted">{heading.detail}</p> : null}<div className="flex flex-wrap gap-2"><StatusPill status={row.protocolVersion.reviewStage} />{recycledKeys.has(`protocol:${row.protocolVersion.protocolId}`) ? <Badge tone="warning">In Recycle Bin</Badge> : null}</div></li>; })}</ul></CardBody></Card>

        <Card><CardHeader title="Attachments" eyebrow="Images, videos and instrument files" /><CardBody className="space-y-4"><AttachmentUploadForm targetType="experiment" targetId={experiment.id} hideTargetFields />{attachmentLinks.length ? <ul className="space-y-2 border-t border-hairline pt-4">{attachmentLinks.map((link) => <li key={link.id}><Link href={`/api/attachments/${link.attachment.id}`} className="break-all text-sm font-medium text-moss hover:underline">{link.attachment.originalFilename}</Link> <span className="text-xs text-muted">· {(link.attachment.size / 1024).toFixed(1)} KB</span></li>)}</ul> : null}</CardBody></Card>
      </aside>
    </div>
  </div></AppShell>;
}
