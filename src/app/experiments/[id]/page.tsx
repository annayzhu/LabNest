...
function Control({ label, children }: { label: string; children: React.ReactNode }) { return <div className="min-w-0"><p className="text-[10px] font-semibold uppercase leading-3 tracking-[0.07em] text-muted">{label}</p><div className="mt-0.5 min-w-0 break-words text-xs font-medium leading-4 text-ink">{children}</div></div>; }

function CompactStat({ label, value }: { label: string; value: React.ReactNode }) { return <span className="whitespace-nowrap text-[10px] leading-4 text-muted"><span className="font-semibold uppercase tracking-[0.04em]">{label}</span><strong className="ml-1 font-medium text-ink">{value}</strong></span>; }
