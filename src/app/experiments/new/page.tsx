...
export default async function NewExperimentPage({ searchParams }: { searchParams?: PageSearchParams }) {
  ...
  const [plansRaw, protocolVersionsRaw, existingExperimentCodes, counter] = await Promise.all([
    ...
    prisma.protocolVersion.findMany({ where: { protocol: { availability: { not: "archived" } } }, include: { protocol: { include: { researchPlans: { select: { researchPlanId: true } }, project: { select: { name: true } } } } }, orderBy: [{ protocol: { title: "asc" } }, { revision: "desc" }] }),
  ...
  const protocolVersions = protocolVersionsRaw.map((version) => ({ id: version.id, researchPlanIds: version.protocol.researchPlans.map(link=>link.researchPlanId), revision: version.revision, displayVersion: version.displayVersion, versionTitle: version.title, reviewStage: version.reviewStage, stepCount: Array.isArray(version.stepsJson) ? version.stepsJson.length : 0, protocol: { id: version.protocol.id, humanCode: version.protocol.humanCode, title: version.protocol.canonicalTitle ?? version.protocol.title, scope: version.protocol.scope, projectName: version.protocol.project?.name ?? null } }));
