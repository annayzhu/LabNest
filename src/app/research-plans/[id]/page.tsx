...
export default async function ResearchPlanDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!plan) notFound();
  const reportSourceReferences = await prisma.reportSource.count({ where: { sourceType: "research_plan", sourceId: plan.id } });
  ...
  return (
    <AppShell><div className="space-y-6">
      <PageHeader identifier={plan.code ?? undefined} eyebrow={plan.project.name} title={plan.title} actions={<><Link href={`/experiments/new?plan=${plan.id}`} className={primaryButton}>New Experiment</Link><Link href={`/research-plans/${plan.id}/edit`} className={secondaryButton}>Edit plan</Link><RecordLifecycleControl id={plan.id} identifier={plan.code} title={plan.title} recordLabel="Research Plan" recordLabelZh="研究方案" blockers={deletionBlockers} archived={plan.status === "archived"} deleteAction={deleteResearchPlan} archiveAction={archiveResearchPlan} editHref={`/research-plans/${plan.id}/edit`} /></>} />
      <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_24rem] 2xl:grid-cols-[minmax(0,1fr)_26rem]">
        ...
        <aside className="order-1 space-y-5 xl:order-2">
          <Card><CardHeader title="Plan control" eyebrow="Compact source map" action={<StatusPill status={plan.status} />} /><CardBody className="space-y-4">
            ...
            <ControlKeyInformationForm id={plan.id} initialValue={plan.keyInformation} scope="research_plan" action={updateResearchPlanKeyInformation} />
          ...
          <Card><CardHeader title="Associated Protocols" eyebrow="Reusable methods" /><CardBody><DataTable rows={plan.protocols} getRowKey={(row) => row.protocolId} emptyMessage="No protocols are associated with this plan." columns={[
            { key: "protocol", header: "Protocol", render: (row) => <div><Link href={`/protocols/${row.protocolId}`} className="font-semibold text-moss hover:underline">{row.protocol.humanCode ?? row.protocol.title}</Link><div className="mt-2 flex flex-wrap items-center gap-2"><Badge tone={row.protocol.scope === "project" ? "sage" : "info"}>{row.protocol.scope}</Badge><span className="font-mono text-xs text-muted">v{row.protocol.versions[0]?.displayVersion ?? "—"}</span></div></div> },
