    ...
    <AppShell><div className="space-y-6">
      ...
      <div className="document-preview-layout">
        ...
        <aside className="document-preview-sidebar">
          ...
          <Card><CardHeader title="Associated Protocols" eyebrow="Reusable methods" /><CardBody><Link href={`/research-plans/${plan.id}/edit`} className="inline-flex min-h-11 items-center text-moss">管理全部（{plan.protocols.length}）</Link><DataTable rows={plan.protocols.slice(0,3)} getRowKey={(row) => row.protocolId} emptyMessage="No protocols are associated with this plan." columns={[
          ...
          <Card><CardHeader title="Experiments" eyebrow="Repeated executions" action={<Link href={`/experiments/new?plan=${plan.id}`} className="text-sm font-medium text-moss hover:underline">Create</Link>} /><CardBody><Link href={`/experiments?plan=${plan.id}`} className="inline-flex min-h-11 items-center text-moss">查看全部（{plan._count.experiments}）</Link><DataTable rows={plan.experiments.slice(0,3)} getRowKey={(row) => row.id} emptyMessage="No experiments have been created for this plan." columns={[
