...
export function AppearancePreview(){
 ...
 <div className="grid grid-cols-2 gap-2"><button type="button" className="appearance-tool" onClick={()=>document.getElementById('appearance-example-volume')?.focus()}><Calculator size={16}/>{zh?'计算器':'Calculator'}</button><button type="button" className="appearance-tool" aria-pressed={timer} onClick={()=>setTimer(!timer)}><Timer size={16}/>{timer?'04:59':zh?'计时器':'Timer'}</button></div>
