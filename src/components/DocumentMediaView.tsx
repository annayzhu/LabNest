...
export function DocumentMediaView({ block }: { block: DocumentMedia }) {
  ...
  if (block.mediaType === "image") return <figure className="ln-protocol-media-preview" style={{ width: `${block.widthPercent ?? 100}%`, maxWidth: "100%" }}>
    <ProtocolMediaImage href={`${documentMediaUrl(block, true)!}${block.attachmentId ? "&preview=1" : ""}`} originalHref={documentMediaUrl(block, true)!} label={block.caption || label} />
