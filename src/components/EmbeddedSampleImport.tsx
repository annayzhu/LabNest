...
export function EmbeddedSampleImport({ frame, toolId }: { frame: RefObject<HTMLIFrameElement | null>; toolId: string }) {
  ...
  return <details className="border-y border-hairline py-2"><summary className="focus-ring min-h-11 cursor-pointer py-2 text-sm">导入样本名单文件</summary>
    <label className="block text-sm">CSV / TSV / XLSX<input type="file" aria-label="样本名单文件" accept=".csv,.tsv,.txt,.xlsx" className="block min-h-11 max-w-full py-2" onChange={async event => {
      ...
      try { if (file.size > 10 * 1024 * 1024) throw Error('名单文件不能超过10MB');
        const data = file.name.endsWith('.xlsx') ? (await (await import('read-excel-file/browser')).readSheet(file)).map(row => row.map(value => value == null ? '' : String(value))) : parseSpreadsheetClipboard(await file.text(), /\.(tsv|txt)$/i.test(file.name) ? '\t' : ',');
