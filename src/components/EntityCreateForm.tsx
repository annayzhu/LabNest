...
export function EntityCreateForm({ action, projects, schemas, initialType = "other", initialMixtureKind, initialProjectId }: {
  ...
  </CardBody></Card><div className="flex items-center justify-end gap-3">{state.error ? <p role="alert" className="text-sm text-error">{state.error}</p> : null}<Button type="submit" variant="primary" size="md" disabled={pending} aria-busy={pending}>{pending ? "Creating…" : "Create Entity"}</Button></div></form>;
}
