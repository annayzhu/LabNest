...
export function MobileMeasurementCapture({ experimentId, step }: { experimentId: string; step?: { id: string; title: string; order: number } }) {
  ...
  return <>
    ...
    {hasOpened && step ? createPortal(<div hidden={!open} className="ln-modal-layer fixed inset-0 z-50 lg:hidden"><button type="button" aria-label="Close measurement" onClick={() => { setOpen(false); trigger.current?.focus(); }} className="ln-modal-backdrop absolute inset-0 bg-ink/25" /><section role="dialog" aria-modal="true" aria-labelledby="measurement-title" className="ln-modal-card ln-modal-sheet absolute inset-x-0 bottom-0 max-h-[88dvh] overflow-y-auto rounded-t-[var(--ln-radius-panel)] border-t border-hairline bg-surface p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] shadow-soft">
