  ...
  return <figure className="space-y-2">
    ...
    <ResizableTableFrame storageKey={`protocol-block:${block.id}`} className="max-h-[480px] overflow-auto editorial-scrollbar">
      <table className="document-three-line-table min-w-full table-fixed text-left text-sm">
        ...
        {headerRow.length ? <thead><tr>{Array.from({ length: columnCount }).map((_, columnIndex) => <th key={columnIndex} data-resizable-column-cell={columnIndex} scope="col" className={cn("min-w-[var(--ln-document-table-min-col-width)] px-[var(--ln-document-table-cell-padding-x)] py-[var(--ln-document-table-cell-padding-y)] pr-4 align-top font-semibold", block.cellColors?.[0]?.[columnIndex] === "risk" && "text-error")} style={block.cellFontSizesPt?.[0]?.[columnIndex] ? { fontSize: `${block.cellFontSizesPt[0][columnIndex]}pt` } : undefined}><TiptapCellContentView content={block.cellRichContent?.[0]?.[columnIndex]} fallback={headerRow[columnIndex] ?? ""} /><span data-column-resize-handle={columnIndex} data-min-width="var(--ln-document-table-min-col-width)" aria-hidden /></th>)}</tr></thead> : null}
        <tbody>{bodyRows.map((row, rowIndex) => <tr key={`${block.id}-${rowIndex + 1}`}>{Array.from({ length: columnCount }).map((_, columnIndex) => <td key={columnIndex} className={cn("min-w-[var(--ln-document-table-min-col-width)] px-[var(--ln-document-table-cell-padding-x)] py-[var(--ln-document-table-cell-padding-y)] align-top", block.cellColors?.[rowIndex + 1]?.[columnIndex] === "risk" && "text-error")} style={block.cellFontSizesPt?.[rowIndex + 1]?.[columnIndex] ? { fontSize: `${block.cellFontSizesPt[rowIndex + 1][columnIndex]}pt` } : undefined}><TiptapCellContentView content={block.cellRichContent?.[rowIndex + 1]?.[columnIndex]} fallback={row[columnIndex] ?? ""} /></td>)}</tr>)}</tbody>
      </table>
    </ResizableTableFrame>
  </figure>;
}
