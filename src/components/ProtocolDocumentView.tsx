...
export function ProtocolContentBlockView({ block }: { block: ProtocolContentBlock }) {
  ...
  return <figure className="space-y-2">
    {block.caption ? <figcaption className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.08em] text-muted"><Table2 className="h-4 w-4" aria-hidden />{block.caption}</figcaption> : null}<ResizableTableFrame storageKey={`protocol-block:${block.id}`} className="max-h-[480px] overflow-auto editorial-scrollbar"><table className="document-three-line-table min-w-full table-fixed text-left text-sm"><colgroup>{Array.from({ length: columnCount }).map((_, columnIndex) => <col key={columnIndex} data-resizable-column-index={columnIndex} style={{ width: block.columnWidths?.[columnIndex] ? `${block.columnWidths[columnIndex]}px` : "var(--ln-document-table-col-width)" }} />)}</colgroup>{headerRow.length ? <thead><tr>{Array.from({ length: columnCount }).map((_, columnIndex) => <th key={columnIndex} data-resizable-column-cell={columnIndex} scope="col" className="min-w-[var(--ln-document-table-min-col-width)] px-[var(--ln-document-table-cell-padding-x)] py-[var(--ln-document-table-cell-padding-y)] pr-4 align-top font-semibold" style={block.cellFontSizesPt?.[0]?.[columnIndex] ? { fontSize: `${block.cellFontSizesPt[0][columnIndex]}pt` } : undefined}>{headerRow[columnIndex] ?? ""}<span data-column-resize-handle={columnIndex} data-min-width="var(--ln-document-table-min-col-width)" aria-hidden /></th>)}</tr></thead> : null}<tbody>{bodyRows.map((row, rowIndex) => <tr key={`${block.id}-${rowIndex + 1}`}>{Array.from({ length: columnCount }).map((_, columnIndex) => <td key={columnIndex} className="min-w-[var(--ln-document-table-min-col-width)] px-[var(--ln-document-table-cell-padding-x)] py-[var(--ln-document-table-cell-padding-y)] align-top" style={block.cellFontSizesPt?.[rowIndex + 1]?.[columnIndex] ? { fontSize: `${block.cellFontSizesPt[rowIndex + 1][columnIndex]}pt` } : undefined}>{row[columnIndex] ?? ""}</td>)}</tr>)}</tbody></table></ResizableTableFrame>
  </figure>;
}
