...
function RichTextContent({ nodes }: { nodes: ProtocolRichTextNode[] }) {
  ...
  for (let index = 0; index < nodes.length;) {
    ...
    if (node.type === "bullet" || node.type === "numbered") {
      ...
      rendered.push(<List key={`list-${index}`} className={type === "bullet" ? "list-disc space-y-1 pl-6" : "list-decimal space-y-1 pl-6"}>{items.map((item, itemIndex) => <li key={itemIndex} data-labnest-line-height={item.lineHeight} style={item.lineHeight ? { lineHeight: item.lineHeight } : undefined}><NodeContent node={item} /></li>)}</List>);
      continue;
    }
    const lineProps = node.lineHeight ? { "data-labnest-line-height": node.lineHeight, style: { lineHeight: node.lineHeight } } : {};
    if (node.type === "heading2") rendered.push(<h3 key={index} {...lineProps} className="document-content-heading pt-2 font-serif font-medium text-ink"><NodeContent node={node} /></h3>);
    else if (node.type === "heading3") rendered.push(<h4 key={index} {...lineProps} className="document-content-heading pt-1 font-semibold text-ink"><NodeContent node={node} /></h4>);
  ...
  return <div className="document-copy space-y-1 text-graphite">{rendered}</div>;
}

export function ProtocolContentBlockView({ block }: { block: ProtocolContentBlock }) {
  if (block.type === "heading") return <h3 className="document-content-heading pt-2 font-serif font-medium text-ink">{block.text}</h3>;
  ...
  return <figure className="space-y-2">
    ...
    {block.caption ? <figcaption className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.08em] text-muted"><Table2 className="h-4 w-4" aria-hidden />{block.caption}</figcaption> : null}<div className="max-h-[480px] overflow-auto editorial-scrollbar"><table className="document-three-line-table min-w-full text-left text-sm">{headerRow.length ? <thead><tr>{Array.from({ length: columnCount }).map((_, columnIndex) => <th key={columnIndex} scope="col" className="min-w-36 px-3 py-2 align-top font-semibold">{headerRow[columnIndex] ?? ""}</th>)}</tr></thead> : null}<tbody>{bodyRows.map((row, rowIndex) => <tr key={`${block.id}-${rowIndex + 1}`}>{Array.from({ length: columnCount }).map((_, columnIndex) => <td key={columnIndex} className="min-w-36 px-3 py-2 align-top">{row[columnIndex] ?? ""}</td>)}</tr>)}</tbody></table></div>
  </figure>;
}
