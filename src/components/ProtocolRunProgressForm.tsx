...
const secondaryButton = "focus-ring inline-flex h-10 items-center justify-center gap-2 rounded-[8px] border border-hairline bg-surface px-4 text-sm font-medium text-moss hover:bg-warm disabled:cursor-wait disabled:opacity-50";
const primaryButton = "focus-ring inline-flex h-10 items-center justify-center gap-2 rounded-[8px] border border-moss bg-moss px-4 text-sm font-medium text-warm disabled:cursor-wait disabled:opacity-50";
...
export function ProtocolRunProgressForm({ experimentId, status, steps, editable }: {
  ...
  return <form action={formAction} className="space-y-4">
    ...
    <section className="overflow-hidden rounded-[12px] border border-hairline bg-surface">
          ...
          return <section key={group.key} aria-labelledby={`run-group-${group.key}`}>
            ...
            <div className="divide-y divide-hairline">
              {group.steps.map((step) => <div key={step.id} className="grid gap-3 px-4 py-4 md:grid-cols-[minmax(0,1fr)_minmax(220px,0.45fr)]">
                ...
                <div>
                  <label><span className={labelClass}>Deviation or incident</span><textarea name={`deviation:${step.id}`} defaultValue={step.deviationNote ?? ""} disabled={!editable || pending} placeholder="Only record what differed from the planned method" className={`${fieldClass} min-h-20 resize-y`} /></label>
      ...
      <div className="px-4 pb-4 pt-3"><label><span className={labelClass}>Execution notes</span><textarea key={state.savedAt ?? "initial"} name="quickNote" disabled={!editable || pending} placeholder="What happened at this stage? This is timestamped and appended to execution notes." className={`${fieldClass} min-h-24 resize-y`} /></label></div>
    </section>
