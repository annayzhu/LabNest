        ...
        {dataset.columns.length ? <ResizableTableFrame storageKey={`result-dataset-editor:${dataset.key}`} className="ln-result-dataset-editor w-full max-w-full overflow-x-auto overscroll-x-contain editorial-scrollbar">
          <table className="min-w-full table-fixed border-collapse text-left text-sm">
            ...
            <tbody>{displayRows.map((row, rowIndex) => <tr key={rowIndex} className="border-b border-hairline last:border-b-0">{dataset.columns.map((column, columnIndex) => <td key={column.key} data-label={`${column.label}${column.unit ? ` (${column.unit})` : ""}${column.required ? " *" : ""}`} className="px-[var(--ln-result-dataset-cell-padding-x)] py-[var(--ln-result-dataset-cell-padding-y)] align-top"><DatasetCellInput column={column} value={row[column.key]} onChange={(value) => {
              ...
              <button type="button" aria-label={`删除第 ${rowIndex + 1} 行`} onClick={() => commitRows(dataset.key, displayRows, displayRows.filter((_, index) => index !== rowIndex))} className="focus-ring rounded-[6px] p-1.5 text-muted hover:bg-error-surface hover:text-error"><Trash2 className="h-4 w-4" /></button>
            </div></td></tr>)}</tbody>
          </table>
...
export function ResultDatasetTableView({ datasets, values, showHeading = true, compactDocument = false }: {
  ...
  return <section className={`min-w-0 max-w-full overflow-hidden ${compactDocument ? "space-y-2" : "space-y-4"}`} aria-label="实验结果数据表 / Result data tables" data-testid="result-dataset-table-view">
    ...
    {datasets.map((dataset) => {
      ...
      return <div key={dataset.key} className={`min-w-0 max-w-full overflow-hidden border border-hairline bg-surface ${compactDocument ? "rounded-[6px]" : "rounded-[9px]"}`}>
