...
import { applySpreadsheetPaste, isRectangularSpreadsheetPaste, type ResultDatasetPasteResult } from "@/lib/result-dataset-paste";
...
const cellInputClass = "focus-ring h-[var(--ln-result-dataset-input-height)] min-w-[var(--ln-result-dataset-input-min-width)] w-full rounded-[var(--ln-radius-control-sm)] border border-hairline bg-surface px-[var(--ln-result-dataset-input-padding-x)] text-[length:var(--ln-result-dataset-font-size)] text-ink";
...
export function ResultDatasetTableEditor({ datasets, values, onChange }: {
  ...
  const tableHistoryRef = useRef<Record<string, ResultDatasetRowValues[][]>>({});
  if (!datasets.length) return null;

  function updateRows(datasetKey: string, rows: ResultDatasetRowValues[]) {
    tableHistoryRef.current[datasetKey] = [];
    onChange({ ...values, [datasetKey]: rows });
  }

  function commitRows(datasetKey: string, currentRows: ResultDatasetRowValues[], nextRows: ResultDatasetRowValues[]) {
    const history = tableHistoryRef.current[datasetKey] ?? [];
    tableHistoryRef.current[datasetKey] = [...history.slice(-19), currentRows.map((row) => ({ ...row }))];
    onChange({ ...values, [datasetKey]: nextRows });
  }

  function undoRows(datasetKey: string) {
    const history = tableHistoryRef.current[datasetKey] ?? [];
    const previous = history.at(-1);
    if (!previous) return false;
    tableHistoryRef.current[datasetKey] = history.slice(0, -1);
    onChange({ ...values, [datasetKey]: previous });
    return true;
  }
  ...
  function moveRow(datasetKey: string, rows: ResultDatasetRowValues[], rowIndex: number, direction: -1 | 1) {
    ...
    commitRows(datasetKey, rows, nextRows);
  }
  ...
  return <div className="min-w-0 max-w-full space-y-4 overflow-hidden" data-testid="result-dataset-table-editor">
    {datasets.map((dataset) => {
      ...
      return <section key={dataset.key} className="min-w-0 max-w-full overflow-hidden rounded-[var(--ln-radius-panel-inner)] border border-hairline bg-surface" data-testid={`result-dataset-entry-${dataset.key}`}>
        <header className="flex flex-wrap items-center justify-between gap-2 border-b border-hairline bg-warm/65 px-3 py-2">
        ...
        {dataset.columns.length ? <ResizableTableFrame storageKey={`result-dataset-editor:${dataset.key}`} className="ln-result-dataset-editor w-full max-w-full overflow-x-auto overscroll-x-contain editorial-scrollbar">
          <table className="min-w-full table-fixed border-collapse text-left text-sm">
            ...
            <tbody>{displayRows.map((row, rowIndex) => <tr key={rowIndex} className="border-b border-hairline last:border-b-0">{dataset.columns.map((column, columnIndex) => <td key={column.key} data-label={`${column.label}${column.unit ? ` (${column.unit})` : ""}${column.required ? " *" : ""}`} className="px-[var(--ln-result-dataset-cell-padding-x)] py-[var(--ln-result-dataset-cell-padding-y)] align-top"><DatasetCellInput column={column} value={row[column.key]} onChange={(value) => {
              ...
              if (!clipboardText || !isRectangularSpreadsheetPaste(clipboardText)) return;
              ...
              commitRows(dataset.key, displayRows, result.rows);
              setPasteNotices((current) => ({ ...current, [dataset.key]: describePasteResult(result) }));
            }} onUndo={() => undoRows(dataset.key)} /></td>)}<td data-row-actions className="sticky right-0 bg-surface px-[var(--ln-result-dataset-cell-padding-x)] py-[var(--ln-result-dataset-cell-padding-y)] align-middle"><div className="flex items-center justify-end gap-1">
              <button type="button" aria-label={`上移第 ${rowIndex + 1} 行`} disabled={rowIndex === 0 || displayRows.length <= 1} onClick={() => moveRow(dataset.key, displayRows, rowIndex, -1)} className="focus-ring rounded-[var(--ln-radius-control-sm)] p-1.5 text-muted hover:bg-stone hover:text-ink disabled:cursor-not-allowed disabled:opacity-30"><ArrowUp className="h-4 w-4" /></button>
              ...
              <button type="button" aria-label={`删除第 ${rowIndex + 1} 行`} onClick={() => commitRows(dataset.key, displayRows, displayRows.filter((_, index) => index !== rowIndex))} className="focus-ring rounded-[var(--ln-radius-control-sm)] p-1.5 text-muted hover:bg-error-surface hover:text-error"><Trash2 className="h-4 w-4" /></button>
            </div></td></tr>)}</tbody>
          </table>
...
export function ResultDatasetTableView({ datasets, values, showHeading = true, compactDocument = false }: {
  datasets: ResultTemplateDataset[];
  values: ResultDatasetValues;
  showHeading?: boolean;
  ...
  if (!datasets.length) return null;
  return <section className={`min-w-0 max-w-full overflow-hidden ${compactDocument ? "space-y-2" : "space-y-4"}`} aria-label="实验结果数据表 / Result data tables" data-testid="result-dataset-table-view">
    ...
    {datasets.map((dataset) => {
      ...
      return <div key={dataset.key} className={`min-w-0 max-w-full overflow-hidden border border-hairline bg-surface ${compactDocument ? "rounded-[var(--ln-radius-control-sm)]" : "rounded-[var(--ln-radius-panel-inner)]"}`}>
        {!compactDocument || datasets.length > 1 ? <div className={`flex flex-wrap items-center justify-between gap-2 border-b border-hairline bg-warm/65 ${compactDocument ? "px-2 py-1.5" : "px-4 py-3"}`}><div><h3 className={compactDocument ? "text-xs font-semibold text-ink" : "text-sm font-semibold text-ink"}>{dataset.label}</h3>{!compactDocument ? <p className="mt-0.5 text-xs text-muted">{validation.rowCount} 条记录 / rows</p> : null}</div>{!compactDocument ? <Badge tone={validation.status === "invalid" ? "danger" : validation.rowCount ? "success" : dataset.required ? "warning" : "neutral"}>{validation.status === "invalid" ? "需修正 / Invalid" : validation.rowCount ? "已填写 / Recorded" : "未填写 / Empty"}</Badge> : null}</div> : null}
        <ResizableTableFrame storageKey={`result-dataset-view:${dataset.key}`} className="ln-result-dataset-view w-full max-w-full overflow-x-auto overscroll-x-contain editorial-scrollbar"><table className="min-w-full table-fixed border-collapse text-left text-[length:var(--ln-result-dataset-font-size)] leading-[1.2]"><colgroup>{dataset.columns.map((column, columnIndex) => <col key={column.key} data-resizable-column-index={columnIndex} style={{ width: column.dataType === "number" ? "var(--ln-result-dataset-view-number-col-width)" : "var(--ln-result-dataset-view-text-col-width)" }} />)}</colgroup><thead><tr className="border-b border-hairline bg-stone/55">{dataset.columns.map((column, columnIndex) => <th key={column.key} data-resizable-column-cell={columnIndex} scope="col" className="min-w-[var(--ln-result-dataset-min-col-width)] px-[var(--ln-result-dataset-cell-padding-x)] py-[var(--ln-result-dataset-cell-padding-y)] pr-4 text-[length:var(--ln-result-dataset-font-size)] font-semibold leading-[1.15] text-ink">{column.label}{column.unit ? <span className="ml-1 font-normal text-muted">({column.unit})</span> : null}<span data-column-resize-handle={columnIndex} data-min-width="var(--ln-result-dataset-min-col-width)" aria-hidden /></th>)}</tr></thead><tbody>{validation.rows.length ? validation.rows.map((row, rowIndex) => <tr key={rowIndex} className="border-b border-hairline text-graphite last:border-b-0">{dataset.columns.map((column) => <td key={column.key} className="max-w-56 whitespace-pre-wrap px-[var(--ln-result-dataset-cell-padding-x)] py-[var(--ln-result-dataset-cell-padding-y)] align-top">{formatDatasetValue(row[column.key], column)}</td>)}</tr>) : <tr><td colSpan={Math.max(dataset.columns.length, 1)} className="px-3 py-5 text-center text-sm text-muted">尚未记录表格数据。</td></tr>}</tbody></table></ResizableTableFrame>
...
function DatasetCellInput({ column, value, onChange, onPaste, onUndo }: { column: ResultDatasetColumn; value: unknown; onChange: (value: unknown) => void; onPaste: (event: ClipboardEvent<HTMLInputElement | HTMLSelectElement>) => void; onUndo: () => boolean }) {
  ...
  const common = { "aria-label": column.label, className: cellInputClass, onKeyDown };
