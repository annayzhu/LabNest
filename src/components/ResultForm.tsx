...
function ResultCaptureEditor({
  ...
  return <section className="document-section" data-testid="result-capture-editor">
    ...
    {template ? <div className="space-y-4">
      {availableModules.length ? <details open className="rounded-[8px] border border-hairline bg-warm/45"><summary className="cursor-pointer px-3 py-2 text-xs font-medium text-graphite">结果内容 <span className="ml-1 text-muted">{selectedModuleIds.length}/{availableModules.length}</span></summary><div className="grid gap-1 border-t border-hairline px-3 py-2 sm:grid-cols-2">{availableModules.map((module) => <label key={module.id} className="flex min-w-0 cursor-pointer items-start gap-2 rounded-[6px] px-1.5 py-1.5 hover:bg-surface"><input type="checkbox" checked={selectedModuleIds.includes(module.id)} onChange={(event) => onSelectedModuleIdsChange(event.target.checked ? [...selectedModuleIds, module.id] : selectedModuleIds.filter((id) => id !== module.id))} className="mt-0.5 h-3.5 w-3.5 accent-moss" /><span className="min-w-0"><span className="block break-words text-xs font-medium leading-5 text-ink">{module.template.title}</span><span className="block break-words text-[11px] leading-4 text-muted">{module.protocolTitle}{module.protocolCode ? <span className="ml-1 font-mono text-[10px]">· {module.protocolCode} · v{module.displayVersion}</span> : null}</span></span></label>)}</div></details> : null}
      {template.instructions?.length ? <aside className="rounded-[8px] border border-sage/35 bg-sage-surface/35 px-3 py-2.5" aria-label="Result template instructions"><p className="mb-1.5 text-[11px] font-semibold uppercase tracking-[0.08em] text-muted">填写说明 / Instructions</p><div className="text-xs leading-5 text-graphite"><ProtocolRichTextContent nodes={template.instructions} /></div></aside> : null}
