...
const datasetSchemaCellClass = "focus-ring h-[var(--ln-template-dataset-input-height)] rounded-[var(--ln-radius-control-sm)] border border-hairline bg-warm px-[var(--ln-template-dataset-input-padding-x)] text-[length:var(--ln-template-dataset-font-size)] leading-none text-ink";
  ...
  return <div className="mt-[var(--ln-result-template-root-margin-top)] space-y-[var(--ln-result-template-section-gap)]">
    ...
    <div className={`grid gap-[var(--ln-result-template-field-grid-gap)] md:grid-cols-2 ${panelClass}`}>
      ...
      <fieldset className="md:col-span-2">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-x-5 gap-y-2 rounded-[var(--ln-radius-control-lg)] border border-hairline bg-surface px-3 py-2.5 text-xs text-graphite">
        ...
        return <article key={`dataset-${datasetIndex}`} className={cardClass}>
          ...
          <div className="mt-[var(--ln-result-template-advanced-margin-top)] overflow-x-auto border-t border-hairline pt-[var(--ln-result-template-advanced-padding-top)]">
            <div className="min-w-[var(--ln-template-dataset-min-width)] space-y-[var(--ln-template-dataset-row-gap)]">
              ...
              {dataset.columns.map((column, columnIndex) => <div key={`dataset-${datasetIndex}-column-${columnIndex}`} className={`grid ${datasetSchemaGridClass} items-center gap-[var(--ln-template-dataset-row-gap)] rounded-[var(--ln-radius-control-sm)] bg-warm/70 px-[var(--ln-template-dataset-row-padding-x)] py-[var(--ln-template-dataset-row-padding-y)]`}>
              ...
              <button type="button" onClick={() => updateDataset(datasetIndex, { ...dataset, columns: [...dataset.columns, defaultColumn(dataset.columns.length, `${t("Column")} ${dataset.columns.length + 1}`)] }, true)} className="focus-ring inline-flex h-7 items-center gap-1 rounded-[var(--ln-radius-control-sm)] border border-hairline bg-surface px-2 text-xs font-medium text-ink hover:bg-stone"><Plus className="h-3.5 w-3.5" />Add column</button>
            </div>
          </div>
