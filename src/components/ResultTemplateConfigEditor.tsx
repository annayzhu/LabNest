...
const inputClass = "focus-ring mt-1 h-9 w-full rounded-[7px] border border-hairline bg-warm px-2.5 text-xs text-ink";
const compactSelectClass = "focus-ring h-9 rounded-[7px] border border-hairline bg-warm px-2.5 text-xs text-ink";
...
export function ResultTemplateConfigEditor({ block, onChange }: { block: TableBlock; onChange: (block: TableBlock) => void }) {
  ...
  return <div className="mt-3 space-y-3">
    ...
    <div className="grid gap-3 rounded-[9px] border border-hairline bg-warm/50 p-3 md:grid-cols-2">
    ...
    <section className="rounded-[9px] border border-hairline bg-warm/50 p-3">
      <div className="mb-2">
    ...
    <section className="space-y-3 rounded-[9px] border border-hairline bg-warm/50 p-3">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex items-center gap-2"><label><span className="sr-only">Evidence type</span><select value={newEvidenceType} onChange={(event) => setNewEvidenceType(event.target.value as EvidenceKind)} className={compactSelectClass}>{evidenceOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label><button type="button" onClick={addEvidence} className="focus-ring inline-flex h-9 items-center gap-1 rounded-[7px] border border-hairline bg-surface px-3 text-xs font-medium text-ink hover:bg-stone"><Plus className="h-3.5 w-3.5" />Add</button></div>
      </div>

      {template.fields.map((field, index) => {
        const dataType = field.dataType ?? field.type ?? "text";
        const number = index + 1;
        return <article key={`field-${index}`} className="rounded-[8px] border border-hairline bg-surface p-3">
          <div className="mb-2 flex items-center justify-between gap-3"><p className="text-xs font-semibold text-ink"><span className="mr-2 text-muted">{number}.</span>{evidenceTypeLabel(field)}</p><div className="flex items-center gap-1"><button type="button" onClick={() => update({ ...template, fields: moveItem(template.fields, index, -1) })} disabled={index === 0} aria-label="Move item up" className="focus-ring rounded p-1.5 text-muted hover:bg-stone disabled:opacity-30"><ChevronUp className="h-4 w-4" /></button><button type="button" onClick={() => update({ ...template, fields: moveItem(template.fields, index, 1) })} disabled={index === template.fields.length - 1} aria-label="Move item down" className="focus-ring rounded p-1.5 text-muted hover:bg-stone disabled:opacity-30"><ChevronDown className="h-4 w-4" /></button><button type="button" onClick={() => update({ ...template, fields: template.fields.filter((_, itemIndex) => itemIndex !== index) }, true)} aria-label="Remove item" className="focus-ring rounded p-1.5 text-error hover:bg-error-surface"><Trash2 className="h-4 w-4" /></button></div></div>
          <div className="grid items-end gap-3 md:grid-cols-[minmax(0,1.5fr)_minmax(120px,0.7fr)_minmax(90px,0.45fr)_auto]">
            ...
            <label className="mb-2 flex items-center gap-2 whitespace-nowrap text-xs font-medium text-muted"><input type="checkbox" checked={field.required ?? false} onChange={(event) => updateField(index, { ...field, required: event.target.checked })} />Required</label>
          </div>
          {dataType === "number" ? <div className="mt-3 flex flex-wrap items-end gap-3 border-t border-hairline pt-3"><label className="w-28"><span className="text-xs font-semibold text-muted">Minimum</span><input type="number" value={field.validation?.min ?? ""} onChange={(event) => updateField(index, { ...field, validation: { ...field.validation, min: parseOptionalNumber(event.target.value) } })} className={inputClass} /></label><label className="w-28"><span className="text-xs font-semibold text-muted">Maximum</span><input type="number" value={field.validation?.max ?? ""} onChange={(event) => updateField(index, { ...field, validation: { ...field.validation, max: parseOptionalNumber(event.target.value) } })} className={inputClass} /></label><label className="mb-2 flex items-center gap-2 text-xs font-medium text-muted"><input type="checkbox" checked={isFeaturedField(field)} onChange={(event) => updateField(index, { ...field, semanticRole: event.target.checked ? "measurement" : "annotation" })} />Show as a key metric</label></div> : null}
          {dataType === "select" ? <label className="mt-3 block border-t border-hairline pt-3"><span className="text-xs font-semibold text-muted">Options</span><DraftTextInput value={field.options?.join(" | ") ?? ""} onValueChange={(value) => updateField(index, { ...field, options: value.split(/[|;,；，]/).map((item) => item.trim()).filter(Boolean) })} className={inputClass} placeholder="Passed | Hold | Failed" /></label> : null}
          <details className="mt-3 border-t border-hairline pt-2"><summary className="focus-ring cursor-pointer select-none text-xs font-medium text-moss">Advanced</summary><div className="mt-3 grid gap-3 md:grid-cols-2"><label><span className="text-xs font-semibold text-muted">Stable key</span><input value={field.key ?? ""} onChange={(event) => updateField(index, { ...field, key: stableResultKey(event.target.value, `field_${index + 1}`) })} className={inputClass} /></label><label><span className="text-xs font-semibold text-muted">Help text</span><DraftTextInput value={field.description ?? ""} onValueChange={(value) => updateField(index, { ...field, description: value || undefined })} className={inputClass} /></label></div></details>
        </article>;
      })}

      {(template.datasets ?? []).map((dataset, datasetIndex) => {
        const itemNumber = template.fields.length + datasetIndex + 1;
        return <article key={`dataset-${datasetIndex}`} className="rounded-[8px] border border-hairline bg-surface p-3">
          <div className="mb-2 flex items-center justify-between gap-3">
            <p className="text-xs font-semibold text-ink"><span className="mr-2 text-muted">{itemNumber}.</span>Data table</p>
            <div className="flex items-center gap-1">
              <button type="button" onClick={() => update({ ...template, datasets: moveItem(template.datasets ?? [], datasetIndex, -1) }, true)} disabled={datasetIndex === 0} aria-label="Move data table up" className="focus-ring rounded p-1.5 text-muted hover:bg-stone disabled:opacity-30"><ChevronUp className="h-4 w-4" /></button>
              <button type="button" onClick={() => update({ ...template, datasets: moveItem(template.datasets ?? [], datasetIndex, 1) }, true)} disabled={datasetIndex === (template.datasets?.length ?? 0) - 1} aria-label="Move data table down" className="focus-ring rounded p-1.5 text-muted hover:bg-stone disabled:opacity-30"><ChevronDown className="h-4 w-4" /></button>
              <button type="button" aria-label="Remove data table" onClick={() => update({ ...template, datasets: (template.datasets ?? []).filter((_, itemIndex) => itemIndex !== datasetIndex) }, true)} className="focus-ring rounded p-1.5 text-error hover:bg-error-surface"><Trash2 className="h-4 w-4" /></button>
            </div>
          </div>
          <div className="grid items-end gap-3 md:grid-cols-[minmax(0,1fr)_auto]"><label><span className="text-xs font-semibold text-muted">Table name</span><DraftTextInput value={dataset.label} onValueChange={(value) => updateDatasetLabel(datasetIndex, value)} className={inputClass} /></label><label className="mb-2 flex items-center gap-2 whitespace-nowrap text-xs font-medium text-muted"><input type="checkbox" checked={dataset.required ?? false} onChange={(event) => updateDataset(datasetIndex, { ...dataset, required: event.target.checked })} />Required</label></div>
          ...
          <details className="mt-3 border-t border-hairline pt-2"><summary className="focus-ring cursor-pointer select-none text-xs font-medium text-moss">Advanced</summary><label className="mt-3 block max-w-sm"><span className="text-xs font-semibold text-muted">Stable table key</span><input value={dataset.key} onChange={(event) => updateDataset(datasetIndex, { ...dataset, key: stableResultKey(event.target.value, `dataset_${datasetIndex + 1}`) })} className={inputClass} /></label></details>
        </article>;
      })}
    ...
    <details className="rounded-[8px] border border-dashed border-hairline bg-surface/70 p-3">
      <summary className="focus-ring cursor-pointer select-none text-xs font-medium text-moss">Advanced template settings</summary>
      <div className="mt-3 grid gap-3 md:grid-cols-2"><label><span className="text-xs font-semibold text-muted">Stable template key</span><input value={template.templateKey ?? ""} onChange={(event) => update({ ...template, templateKey: stableResultKey(event.target.value, "result_template") })} className={inputClass} /></label><div className="rounded-[7px] border border-hairline bg-warm/70 p-3 text-xs leading-5 text-muted">Stable keys are used for imports and Result matching. Existing custom keys remain unchanged unless edited here.</div></div>
      {template.view?.charts?.length ? <div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-[7px] border border-hairline bg-warm/70 p-3"><p className="text-xs leading-5 text-muted">{template.view.charts.length} legacy custom chart rule(s) are preserved for compatibility.</p><button type="button" onClick={() => update({ ...template, view: { ...template.view, charts: [] } })} className="focus-ring h-8 rounded-[7px] border border-hairline bg-surface px-3 text-xs font-medium text-ink hover:bg-stone">Use automatic chart preview</button></div> : null}
    </details>
  </div>;
}
