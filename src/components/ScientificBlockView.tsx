...
export function ScientificBlockView({ block }: { block: ScientificContentBlock }) {
  ...
  if (block.type === "table") {
    ...
    return <ResizableTableFrame storageKey={`scientific-block:${block.id}`} className="overflow-x-auto"><table className="document-three-line-table min-w-full table-fixed text-left text-sm"><colgroup>{Array.from({ length: columnCount }).map((_, columnIndex) => <col key={columnIndex} data-resizable-column-index={columnIndex} style={{ width: "var(--ln-document-table-col-width)" }} />)}</colgroup><caption className="mb-2 text-left text-xs font-semibold uppercase tracking-[0.08em] text-muted">{block.caption}</caption>{headerRow.length ? <thead><tr>{Array.from({ length: columnCount }).map((_, cellIndex) => <th key={`${block.id}-header-${cellIndex}`} data-resizable-column-cell={cellIndex} scope="col" className="max-w-sm whitespace-pre-wrap px-[var(--ln-document-table-cell-padding-x)] py-[var(--ln-document-table-cell-padding-y)] pr-4 align-top font-semibold">{headerRow[cellIndex] ?? ""}<span data-column-resize-handle={cellIndex} data-min-width="var(--ln-document-table-min-col-width)" aria-hidden /></th>)}</tr></thead> : null}<tbody>{bodyRows.map((row, rowIndex) => <tr key={`${block.id}-${rowIndex + 1}`} className="text-graphite">{Array.from({ length: columnCount }).map((_, cellIndex) => <td key={`${block.id}-${rowIndex + 1}-${cellIndex}`} className="max-w-sm whitespace-pre-wrap px-[var(--ln-document-table-cell-padding-x)] py-[var(--ln-document-table-cell-padding-y)] align-top">{row[cellIndex] ?? ""}</td>)}</tr>)}</tbody></table></ResizableTableFrame>;
  }
