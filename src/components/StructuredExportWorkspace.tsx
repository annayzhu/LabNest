      ...
      <Card>
        ...
        <CardBody className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {definition.exportFormats.map((format) => (
            <button key={format} type="submit" name="format" value={format} className="focus-ring flex min-h-24 items-center justify-between gap-3 rounded-[var(--ln-radius-panel-inner)] border border-hairline bg-warm px-4 py-4 text-left transition hover:border-border-strong hover:bg-sage-surface/50">
              <span>
...
function ScopeOption({
  ...
  return (
    <label className={`rounded-[var(--ln-radius-panel-inner)] border p-4 ${disabled ? "cursor-not-allowed border-hairline bg-stone/60 opacity-60" : "cursor-pointer border-hairline bg-warm has-[:checked]:border-moss has-[:checked]:bg-sage-surface/50"}`}>
