...
const controlClass =
  "focus-ring h-8 w-full rounded-[var(--ln-radius-control-md)] border border-hairline bg-white px-2.5 text-xs text-ink placeholder:text-muted";
  ...
  return (
    <label className="grid gap-1 text-xs text-graphite">
      ...
      <textarea
        ...
        className="focus-ring min-h-20 resize-y rounded-[var(--ln-radius-control-md)] border border-hairline bg-[#FBFBF9] px-2.5 py-2 font-mono text-[10px] leading-4 text-ink placeholder:text-muted"
      />
    ...
    <div className="space-y-[var(--ln-vis-panel-gap)]" data-visualization-workbench>
          ...
          <CardBody className="grid gap-1.5 p-2.5">
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              {(Object.keys(paletteSeries) as PaletteSeriesId[]).map((id) => (
                <button key={id} type="button" title={paletteSeries[id].description} onClick={() => selectPaletteSeries(id)} className={cn("focus-ring h-7 rounded-[var(--ln-radius-control-sm)] border px-2 text-[11px] font-medium transition-colors", paletteSeriesId === id ? "border-action-border bg-[var(--ln-vis-active-bg)] font-semibold text-moss" : "border-transparent bg-transparent text-graphite hover:border-hairline hover:bg-warm")}>{paletteSeries[id].name}</button>
              ))}
              ...
              {paletteSeriesId === "custom" ? customPalettes.length ? customPalettes.map((palette) => (
                <button key={palette.id} type="button" title={palette.name} onClick={() => selectCustomPalette(palette)} className={cn("focus-ring flex h-7 items-center gap-1.5 rounded-[var(--ln-radius-control-sm)] border px-2 text-[11px] transition", selectedCustomPaletteId === palette.id ? "border-moss bg-sage-surface text-ink" : "border-hairline bg-white text-graphite hover:border-border-strong")}>
              ...
              )) : <span className="rounded-[var(--ln-radius-control-sm)] border border-dashed border-hairline px-2 py-1 text-[11px] text-muted">No custom palettes</span> : paletteSeries[paletteSeriesId].themeIds.map((id) => (
                <button key={id} type="button" title={journalThemes[id].description} onClick={() => selectTheme(id)} className={cn("focus-ring flex h-7 items-center gap-1.5 rounded-[var(--ln-radius-control-sm)] border px-2 text-[11px] transition", themeId === id ? "border-moss bg-sage-surface text-ink" : "border-hairline bg-white text-graphite hover:border-border-strong") }>
            ...
            <div className="flex flex-wrap items-center gap-1.5">
              <input aria-label="Custom palette name" value={customPaletteName} placeholder="Custom palette name" onChange={(event) => setCustomPaletteName(event.target.value)} className="focus-ring h-7 w-40 rounded-[var(--ln-radius-control-sm)] border border-hairline bg-white px-2 text-[11px] text-ink placeholder:text-muted" />
              ...
              <select aria-label="Figure font" className="focus-ring h-7 rounded-[var(--ln-radius-control-sm)] border border-hairline bg-white px-2 text-[11px] text-graphite" value={settings.fontFamily} onChange={(event) => updateSetting("fontFamily", event.target.value as FigureFontId)}>
        ...
        <Card className="rounded-[var(--ln-vis-panel-radius)] border-[var(--ln-vis-panel-border)] shadow-none md:hidden">
          <CardBody className="space-y-2 p-2">
            <div className="relative">
              ...
              <input aria-label="Search plot types" type="search" value={plotSearchQuery} onChange={(event) => setPlotSearchQuery(event.target.value)} placeholder="Search plots or questions" className="focus-ring h-9 w-full rounded-[var(--ln-radius-control-lg)] border border-hairline bg-white pl-9 pr-3 text-xs text-ink placeholder:text-muted" />
            </div>
            <div className="relative">
              <select aria-label="Plot type" value={plotType} onChange={(event) => selectPlot(event.target.value as PlotType)} className="focus-ring h-10 w-full appearance-none rounded-[var(--ln-radius-control-lg)] border border-hairline bg-white px-3 pr-9 text-sm font-medium text-ink">
            ...
            <button type="button" aria-expanded={plotFinderOpen} onClick={() => setPlotFinderOpen((open) => !open)} className="focus-ring flex w-full items-center justify-between rounded-[var(--ln-radius-control-md)] px-2 py-1.5 text-left text-[11px] font-medium text-moss hover:bg-sage-surface"><span className="flex items-center gap-1.5"><Lightbulb className="h-3.5 w-3.5" aria-hidden />Help me choose a plot</span><ChevronDown className={cn("h-3.5 w-3.5 transition-transform", plotFinderOpen && "rotate-180")} aria-hidden /></button>
            {plotFinderOpen ? <div className="space-y-2 rounded-[var(--ln-radius-control-lg)] border border-hairline bg-stone p-2">
              ...
              <div className="flex flex-wrap gap-1">{recommendedPlotModules.map((candidate) => <button key={`mobile-recommend-${candidate.definition.id}`} type="button" onClick={() => selectPlot(candidate.definition.id)} className="focus-ring rounded-[var(--ln-radius-control-sm)] border border-hairline bg-white px-2 py-1 text-[11px] text-graphite hover:border-moss hover:text-ink">{candidate.definition.name}</button>)}</div>
            </div> : null}
          </CardBody>
        </Card>
      </div>
      ...
      <div className="grid items-start gap-[var(--ln-vis-panel-gap)] xl:grid-cols-[180px_minmax(0,1fr)_288px] xl:items-start" style={mainGridStyle}>
        <Card data-visualization-panel="plots" className="hidden rounded-[var(--ln-vis-panel-radius)] border-[var(--ln-vis-panel-border)] shadow-none md:block xl:sticky xl:top-[var(--visualization-panel-top)] xl:flex xl:h-[var(--visualization-panel-height)] xl:min-h-0 xl:flex-col xl:overflow-hidden">
          <CardHeader title="Plot types" className="h-12 shrink-0" action={<button type="button" aria-expanded={plotFinderOpen} onClick={() => setPlotFinderOpen((open) => !open)} className="focus-ring rounded-[var(--ln-radius-control-sm)] px-1.5 py-1 text-[10px] font-semibold text-moss hover:bg-sage-surface">Choose</button>} />
          <div className="shrink-0 space-y-2 border-b border-hairline p-2">
            <div className="relative">
              ...
              <input aria-label="Search plot types" type="search" value={plotSearchQuery} onChange={(event) => setPlotSearchQuery(event.target.value)} placeholder="Search plots" className="focus-ring h-8 w-full rounded-[var(--ln-radius-control-md)] border border-hairline bg-white pl-8 pr-2 text-[11px] text-ink placeholder:text-muted" />
            </div>
            {plotFinderOpen ? <div className="space-y-1.5 rounded-[var(--ln-radius-control-md)] bg-stone p-2">
              ...
              <select id="plot-finder-goal" aria-label="What do you want to show?" value={plotFinderGoalId} onChange={(event) => setPlotFinderGoalId(event.target.value as PlotFinderGoalId)} className="focus-ring h-8 w-full rounded-[var(--ln-radius-control-sm)] border border-hairline bg-white px-2 text-[11px] text-ink">{plotFinderGoals.map((goal) => <option key={goal.id} value={goal.id}>{goal.label}</option>)}</select>
          ...
          <CardBody className="space-y-1 p-2 xl:min-h-0 xl:flex-1 xl:overflow-y-auto xl:[scrollbar-gutter:stable]">
            ...
            {filteredPlotModules.length === 0 ? <div className="rounded-[var(--ln-radius-control-md)] border border-dashed border-hairline px-2 py-4 text-center text-[11px] leading-4 text-muted">No matching plot. Try a scientific question such as “survival”, “enrichment”, “微生物”, or “相关”.</div> : null}
          </CardBody>
        </Card>
          ...
          <Card className="overflow-hidden rounded-[var(--ln-vis-panel-radius)] border-[var(--ln-vis-preview-border)] shadow-[var(--ln-vis-preview-shadow)]">
            ...
            <CardBody className="p-3 sm:p-4">
              ...
              {validation.errors.length > 0 ? (
                <div className="rounded-[var(--ln-radius-panel-inner)] border border-error/25 bg-error-surface px-3 py-2 text-xs leading-5 text-error">
                  {validation.errors.map((error) => <p key={error}>• {error}</p>)}
                </div>
              ) : (
                <div className="overflow-auto rounded-[var(--ln-radius-control-lg)] border border-[var(--ln-vis-panel-border)] bg-[var(--ln-vis-canvas-bg)] p-2 sm:p-3">
              ...
              {validation.warnings.length > 0 ? <div className="mt-3 rounded-[var(--ln-radius-control-lg)] border border-warning/20 bg-warning-surface px-3 py-2 text-xs leading-5 text-warning">{validation.warnings.join(" · ")}</div> : null}
              {plotType === "bar" && isValid && barAnalysis?.results.length ? <details className="mt-3 overflow-hidden rounded-[var(--ln-radius-control-lg)] border border-hairline bg-white">
            ...
            <CardBody className="grid items-start gap-3 p-3 lg:grid-cols-[minmax(0,1fr)_minmax(240px,0.56fr)] sm:p-4">
              <div className="grid content-start gap-2 text-xs text-graphite">
                  ...
                  <div className="flex flex-wrap gap-1.5" aria-label="Example data">
                    {dataExamples.map((example, exampleIndex) => (
                      <button
                        ...
                        className={cn(
                          "focus-ring h-7 rounded-[var(--ln-radius-control-md)] border px-2 text-[11px] font-medium transition",
                ...
                {rawData.length > 500_000 ? (
                  <div className="min-h-44 overflow-hidden rounded-[var(--ln-radius-control-lg)] border border-hairline bg-[#FBFBF9] p-3">
              ...
              <div className="space-y-2.5">
                ...
                {plotType === "pca" ? <div className="grid grid-cols-2 gap-1 rounded-[var(--ln-radius-control-lg)] border border-hairline bg-stone p-1" aria-label="PCA input mode">
                  <button type="button" aria-pressed={pcaInputMode === "scores"} onClick={() => selectPcaInputMode("scores")} className={cn("focus-ring rounded-[var(--ln-radius-control-sm)] px-2 py-2 text-[11px] font-medium", pcaInputMode === "scores" ? "bg-white text-ink shadow-sm" : "text-muted hover:text-ink")}>Supplied coordinates</button>
                  <button type="button" aria-pressed={pcaInputMode === "matrix"} onClick={() => selectPcaInputMode("matrix")} className={cn("focus-ring rounded-[var(--ln-radius-control-sm)] px-2 py-2 text-[11px] font-medium", pcaInputMode === "matrix" ? "bg-white text-ink shadow-sm" : "text-muted hover:text-ink")}>Calculate from matrix</button>
                </div> : null}
                {plotType === "pca" && pcaAnalysis ? <>
                  ...
                  <div className="rounded-[var(--ln-radius-control-lg)] border border-hairline px-3 py-2 text-[11px] leading-5 text-muted">
                ...
                </> : visibleRoles.length === 0 ? <p className="rounded-[var(--ln-radius-control-lg)] bg-stone px-3 py-2 text-xs leading-5 text-graphite">The first column supplies row labels; all remaining columns form the numeric matrix.</p> : <>{visibleRoles.map((role) => (
                ...
                ))}{plotType === "pca" ? <p className="rounded-[var(--ln-radius-control-lg)] bg-sage-surface px-3 py-2 text-[11px] leading-4 text-graphite">Studio renders the supplied coordinates without recomputing PCA. Add explained variance to the X/Y axis labels when it is available from the upstream workflow.</p> : null}</>}
                <div className="rounded-[var(--ln-radius-control-lg)] border border-hairline px-3 py-2 text-[11px] leading-5 text-muted">
        ...
        <div data-visualization-panel="parameters" className="flex min-h-0 flex-col gap-[var(--ln-vis-panel-gap)] overflow-hidden xl:sticky xl:top-[var(--visualization-panel-top)] xl:h-[var(--visualization-panel-height)]">
          <Card className="min-h-0 rounded-[var(--ln-vis-panel-radius)] border-[var(--ln-vis-panel-border)] shadow-none xl:flex xl:flex-1 xl:flex-col">
            ...
            {(plotType === "venn" || plotType === "upset") ? <ControlGroup title="Set intersections">
              ...
              <p className="rounded-[var(--ln-radius-control-lg)] bg-stone px-3 py-2 text-[11px] leading-4 text-graphite">Counts are exact membership combinations. Peak mode splits half-open intervals [start, end) into disjoint atomic genomic segments wherever active set membership changes; counts are segments, not base pairs or original peaks. Size weighting is only a visual cue, never an area-proportional fit.</p>
            </ControlGroup> : null}
            ...
            {plotType === "roc" ? <ControlGroup title="ROC input">
              ...
              <p className="rounded-[var(--ln-radius-control-lg)] bg-stone px-3 py-2 text-[11px] leading-4 text-graphite">Raw mode computes empirical ROC and trapezoidal AUC per model. Time-dependent mode only displays censoring-aware coordinates, pointwise TPR intervals, horizons, and AUC intervals supplied by a documented upstream method; it never infers them from ordinary binary scores.</p>
            </ControlGroup> : null}

            {plotType === "calibration" ? <ControlGroup title="Calibration grouping"><RangeControl label="Equal-frequency bins" value={settings.calibrationBinCount} minimum={3} maximum={15} step={1} onChange={(value) => updateSetting("calibrationBinCount", value)} /><p className="rounded-[var(--ln-radius-control-lg)] bg-stone px-3 py-2 text-[11px] leading-4 text-graphite">Subjects are sorted by predicted probability and divided into approximately equal-frequency bins separately for each model. Fewer bins are more stable in small cohorts.</p></ControlGroup> : null}

            {plotType === "decision-curve" ? <ControlGroup title="Decision thresholds"><RangeControl label="Minimum threshold" value={settings.decisionThresholdMinimum} minimum={0.005} maximum={0.5} step={0.005} onChange={(value) => updateSetting("decisionThresholdMinimum", value)} /><RangeControl label="Maximum threshold" value={settings.decisionThresholdMaximum} minimum={0.05} maximum={0.99} step={0.01} onChange={(value) => updateSetting("decisionThresholdMaximum", value)} /><RangeControl label="Grid resolution" value={settings.decisionThresholdStep} minimum={0.005} maximum={0.05} step={0.005} onChange={(value) => updateSetting("decisionThresholdStep", value)} /><p className="rounded-[var(--ln-radius-control-lg)] bg-stone px-3 py-2 text-[11px] leading-4 text-graphite">Net benefit is evaluated only on the displayed threshold grid. Choose a clinically meaningful interval; a grid step ≤0.01 is recommended when narrow utility regions matter.</p></ControlGroup> : null}
            ...
            {plotType === "line" ? <ControlGroup title="Pointwise significance">
              ...
              {settings.showSignificance ? <>
                ...
                <p className="rounded-[var(--ln-radius-control-lg)] bg-stone px-3 py-2 text-[11px] leading-4 text-graphite">At each X value, every non-reference series is compared with the selected reference using an independent-samples Welch t test. Map mean, SD or SEM, and an explicit integer n ≥ 2. BH correction is applied across all displayed pointwise comparisons. Do not use this calculation for paired or repeated-measures data; supply results from the appropriate upstream model instead.</p>
              </> : null}
            </ControlGroup> : null}
            ...
            {plotType === "bar" ? <>
              <ControlGroup title="Categorical design">
                ...
                {settings.barVariant === "dual-axis" ? <p className="rounded-[var(--ln-radius-control-lg)] bg-stone px-3 py-2 text-[11px] leading-4 text-graphite">The secondary column uses an independently labelled right-side scale. Use only when the two units are explicit and a shared baseline would be misleading.</p> : null}
                {settings.barVariant === "overlay" ? <p className="rounded-[var(--ln-radius-control-lg)] bg-stone px-3 py-2 text-[11px] leading-4 text-graphite">The secondary column shares the primary value axis and should use the same unit.</p> : null}
              ...
              {settings.barVariant !== "polar" ? <ControlGroup title="Statistical analysis">
                ...
                <p className="rounded-[var(--ln-radius-control-lg)] bg-stone px-3 py-2 text-[11px] leading-4 text-graphite">Independent modes use two-sided Welch t-tests within each group/facet. Summary inference requires explicit integer n ≥ 2 and SD or SEM; n is never inferred. Paired mode requires the same subject IDs in both categories. qPCR displays relative expression but tests biological-replicate ΔCt. Technical replicates must be aggregated before import.</p>
              </ControlGroup> : null}
            ...
            {distributionPlotTypes.includes(plotType) ? <ControlGroup title="Distribution layers">
              ...
              <p className="rounded-[var(--ln-radius-control-lg)] bg-stone px-3 py-2 text-[11px] leading-4 text-graphite">SD describes sample spread, SEM describes mean precision, and the 95% CI uses a two-sided Student t interval. Paired lines require a subject ID. P-value labels display mapped results only; this tool does not choose or run a significance test.</p>
            </ControlGroup> : null}
            ...
            {plotType === "scatter" || plotType === "correlation" ? <ControlGroup title="Association view">
              ...
              <p className="rounded-[var(--ln-radius-control-lg)] bg-stone px-3 py-2 text-[11px] leading-4 text-graphite">Pearson tests linear association; Spearman tests monotonic rank association. Reported P values use a two-sided t approximation and do not adjust for multiple testing. Confidence ribbons are mean-response intervals for linear fits, not prediction intervals. Ternary rows are normalized to proportions. The 3D view independently min–max scales X, Y, and Z before a fixed-size orthographic projection; it preserves order, not cross-axis units or perspective.</p>
            </ControlGroup> : null}
            {plotType === "correlation-heatmap" ? <ControlGroup title="Correlation"><SelectControl label="Method" value={settings.correlationMethod} onChange={(value) => updateSetting("correlationMethod", value as VisualizationSettings["correlationMethod"])}><option value="pearson">Pearson</option><option value="spearman">Spearman</option></SelectControl><p className="rounded-[var(--ln-radius-control-lg)] bg-stone px-3 py-2 text-[11px] leading-4 text-graphite">Each matrix cell is calculated from paired complete observations. This view reports coefficients, not inferential P values.</p></ControlGroup> : null}
            ...
            {plotType === "manhattan" ? <ControlGroup title="Genome-wide threshold"><RangeControl label="Significance −log₁₀(P)" value={settings.genomicSignificanceLog10} minimum={1} maximum={15} step={0.1} onChange={(value) => updateSetting("genomicSignificanceLog10", value)} /><ToggleControl label="Label strongest loci" checked={settings.showLabels} onChange={(value) => updateSetting("showLabels", value)} />{settings.showLabels ? <RangeControl label="Maximum labels" value={settings.labelLimit} minimum={1} maximum={30} step={1} onChange={(value) => updateSetting("labelLimit", value)} /> : null}<p className="rounded-[var(--ln-radius-control-lg)] bg-stone px-3 py-2 text-[11px] leading-4 text-graphite">The threshold is a display reference only. Choose genome-wide significance according to the tested hypothesis family and study design; the tool does not adjust P values.</p></ControlGroup> : null}
            {plotType === "qq" ? <ControlGroup title="QQ labels"><ToggleControl label="Label strongest deviations" checked={settings.showLabels} onChange={(value) => updateSetting("showLabels", value)} />{settings.showLabels ? <RangeControl label="Maximum labels" value={settings.labelLimit} minimum={1} maximum={30} step={1} onChange={(value) => updateSetting("labelLimit", value)} /> : null}<p className="rounded-[var(--ln-radius-control-lg)] bg-stone px-3 py-2 text-[11px] leading-4 text-graphite">Expected quantiles use (i − 0.5) / n. Systematic deviation can reflect polygenicity, confounding, or model misspecification; this view does not estimate genomic inflation by itself.</p></ControlGroup> : null}
            ...
            {plotType === "genome-tracks" ? <ControlGroup title="Genome tracks"><RangeControl label="Track gap" value={settings.genomicTrackGap} minimum={0} maximum={12} step={1} unit=" px" onChange={(value) => updateSetting("genomicTrackGap", value)} /><ToggleControl label="Show feature labels" checked={settings.showLabels} onChange={(value) => updateSetting("showLabels", value)} /><p className="rounded-[var(--ln-radius-control-lg)] bg-stone px-3 py-2 text-[11px] leading-4 text-graphite">All intervals are aligned on one coordinate system. The viewer does not infer reference build, strand, transcript model, or assay normalization.</p></ControlGroup> : null}
            {(["sankey", "alluvial", "chord", "ligand-receptor"] as PlotType[]).includes(plotType) ? <ControlGroup title="Flow semantics"><ToggleControl label="Show compact labels" checked={settings.showLabels} onChange={(value) => updateSetting("showLabels", value)} /><p className="rounded-[var(--ln-radius-control-lg)] bg-stone px-3 py-2 text-[11px] leading-4 text-graphite">Ribbon width uses the supplied non-negative weight. Sankey aggregates repeated source–target–group rows and discloses it; Alluvial requires conserved flow IDs across ordered axes. Ligand–receptor scores remain upstream evidence, not causal communication proof.</p></ControlGroup> : null}
            {plotType === "circos" ? <ControlGroup title="Circos tracks"><RangeControl label="Track gap" value={settings.genomicTrackGap} minimum={0} maximum={12} step={1} unit=" px" onChange={(value) => updateSetting("genomicTrackGap", value)} /><ToggleControl label="Show compact labels" checked={settings.showLabels} onChange={(value) => updateSetting("showLabels", value)} /><p className="rounded-[var(--ln-radius-control-lg)] bg-stone px-3 py-2 text-[11px] leading-4 text-graphite">All record types share explicit chromosome/contig lengths from one reference build. Numeric tracks scale independently and display their ranges; correlation is a signed coefficient in [−1, 1]. link, fusion, and correlation require target intervals and target sequence lengths.</p></ControlGroup> : null}
            {(plotType === "waterfall" || plotType === "oncoplot") ? <ControlGroup title="Cohort ordering"><ToggleControl label="Sort samples by event burden" checked={settings.genomicSortSamples} onChange={(value) => updateSetting("genomicSortSamples", value)} />{plotType === "oncoplot" ? <ToggleControl label="Show burden and frequency margins" checked={settings.oncoplotShowMargins} onChange={(value) => updateSetting("oncoplotShowMargins", value)} /> : null}<p className="rounded-[var(--ln-radius-control-lg)] bg-stone px-3 py-2 text-[11px] leading-4 text-graphite">Rows are input alteration events. Waterfall height is event count, not normalized tumor mutation burden. Oncoplot co-occurrence is descriptive and is not a mutual-exclusivity test.</p></ControlGroup> : null}
            {plotType === "motif-logo" ? <ControlGroup title="Motif scale"><SelectControl label="Letter height" value={settings.motifDisplayMode} onChange={(value) => updateSetting("motifDisplayMode", value as VisualizationSettings["motifDisplayMode"])}><option value="information">Information content (bits)</option><option value="probability">Probability</option></SelectControl><p className="rounded-[var(--ln-radius-control-lg)] bg-stone px-3 py-2 text-[11px] leading-4 text-graphite">Information mode uses R = 2 − H for DNA and letter height p(base) × R. Probabilities must already include any pseudocount treatment; sample-size correction is not inferred.</p></ControlGroup> : null}
            {(["network", "ppi", "cerna", "mirna-target", "cnet", "enrichment-map"] as PlotType[]).includes(plotType) ? <ControlGroup title="Network layout">
              ...
              <p className="rounded-[var(--ln-radius-control-lg)] bg-stone px-3 py-2 text-[11px] leading-4 text-graphite">Node color encodes group and optional value controls size. Edge width encodes weight, arrowheads encode direction, color encodes sign, and dash pattern encodes edge type. Layout coordinates are deterministic for the exported seed but have no biological distance meaning.</p>
            </ControlGroup> : null}
            {(plotType === "tree" || plotType === "dendrogram") ? <ControlGroup title="Hierarchy layout"><SelectControl label="Orientation" value={settings.treeOrientation} onChange={(value) => updateSetting("treeOrientation", value as VisualizationSettings["treeOrientation"])}><option value="vertical">Top to bottom</option><option value="horizontal">Left to right</option></SelectControl><ToggleControl label="Show leaf labels" checked={settings.showLabels} onChange={(value) => updateSetting("showLabels", value)} /><p className="rounded-[var(--ln-radius-control-lg)] bg-stone px-3 py-2 text-[11px] leading-4 text-graphite">Tree branch length encodes hierarchy depth only. Dendrogram branch position uses supplied merge height; horizontal leaf spacing is layout and is not a sample distance.</p></ControlGroup> : null}
            ...
            {plotType === "km" ? <ControlGroup title="Survival"><ToggleControl label="Show numbers at risk" checked={settings.showRiskTable} onChange={(value) => updateSetting("showRiskTable", value)} /><p className="rounded-[var(--ln-radius-control-lg)] bg-stone px-3 py-2 text-[11px] leading-4 text-graphite">Kaplan–Meier estimates and censor marks are calculated from individual records. A log-rank P value is intentionally omitted until a tested inferential module is added.</p></ControlGroup> : null}
