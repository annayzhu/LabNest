...
import {toggleFavorite,recentCalculators} from '@/lib/calculators/calculator-storage';
...
export function CalculatorCatalog({contextQuery=''}:{contextQuery?:string}){
 ...
 const recent=recentCalculators(state?.recent??[]).filter(item=>catalog.some(t=>t.id===item.calculatorId)).slice(0,6);
 return <div className="calculator-catalog"><div className="calculator-catalog-heading"><CalculatorBack href={back} zh={zh} catalog/><h1>{zh?'实验计算':'Lab Calculations'}</h1><label><Search size={18}/><input aria-label={zh?'搜索计算':'Search calculations'} value={query} onChange={e=>search(e.target.value)} placeholder={zh?'搜索工具…':'Search…'}/></label><CalculatorMore zh={zh}/></div>
 ...
 {!query?<><CommonTasks pins={pins} state={state} update={update} zh={zh} contextQuery={contextQuery}/>{recent.length?<section className="calculator-recent"><h2>{zh?'最近使用':'Recently used'}</h2><div className="calculator-recent-grid">{recent.map(item=><CalculatorToolTile key={item.calculatorId} id={item.calculatorId} zh={zh} contextQuery={contextQuery}/>)}</div></section>:null}</>:null}
