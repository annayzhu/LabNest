...
export function CalculatorPresets({zh,presets,disabled,onSave,onLoad,onRename,onDelete}:{
  ...
  return <div className="calculator-presets" aria-label={zh?'常用预设':'Presets'}>
    {presets.length>0&&presets.length<=6?<select aria-label={zh?'常用预设':'Presets'} value="" onChange={e=>{const preset=presets.find(p=>p.id===e.target.value);if(preset)void load(preset);}}><option value="">{zh?'常用预设':'Presets'}</option>{presets.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select>:null}
    <button type="button" disabled={disabled} onClick={()=>void namePreset()}>{zh?'存为预设':'Save as preset'}</button>
    {presets.length>0?<details className="calculator-preset-menu"><summary>{presets.length>6?(zh?'选择 / 管理预设':'Choose / manage presets'):(zh?'管理预设':'Manage presets')}</summary><label>{zh?'搜索预设':'Search presets'}<input value={query} onChange={e=>setQuery(e.target.value)}/></label><div className="calculator-preset-list">{visible.map(p=><div key={p.id}><button type="button" className="calculator-preset-name" onClick={()=>void load(p)}>{p.name}</button><button type="button" disabled={disabled} aria-label={`${zh?'重命名':'Rename'} ${p.name}`} onClick={()=>void namePreset(p)}>{zh?'重命名':'Rename'}</button><button type="button" disabled={disabled} aria-label={`${zh?'删除':'Delete'} ${p.name}`} onClick={async()=>{if(await dialog.confirm({title:zh?'删除此预设？':'Delete this preset?',description:p.name,confirmLabel:zh?'删除':'Delete',cancelLabel}))onDelete(p.id);}}>{zh?'删除':'Delete'}</button></div>)}{!visible.length?<p>{zh?'没有匹配的预设':'No matching presets'}</p>:null}</div></details>:null}
  </div>;
}
