...
export function CalculatorCatalog({contextQuery=""}:{contextQuery?:string}) {
  ...
  const toolLink=(id:string)=>{const tool=catalog.find(t=>t.id===id);return tool?<Link className="block min-h-11 min-w-0 rounded-lg border border-hairline p-3 hover:bg-warm" href={`/tools/calculator/${id}${contextQuery?`?${contextQuery}`:""}`}><strong className="text-sm">{zh?tool.nameZh:tool.name}</strong><p className="mt-1 text-xs text-muted">{zh?tool.shortDescriptionZh:tool.shortDescription}</p></Link>:null;};
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3">
          {!embedded ? <Link href={`/tools/calculator${contextQuery?`?${contextQuery}`:""}`} className="focus-ring flex min-h-11 w-9 shrink-0 items-center justify-center rounded-[var(--ln-radius-control-lg)] border border-hairline text-muted hover:bg-warm hover:text-ink" aria-label={zh ? "返回计算器" : "Back to calculators"}><ArrowLeft className="h-4 w-4" /></Link> : null}
