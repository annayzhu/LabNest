"use client";
import {workspaceLayout, hasPipettingSettings, fieldWidth, orderedFields} from "@/lib/calculators/workspace-presentation";
...
import {reactionMixClipboard} from '@/lib/calculators/reaction-mix-presentation';
...
export function CalculatorWorkbench({ calculatorId, initialInputs = {}, plateContext, embedded = false, embeddedLocale }: { calculatorId: string; initialInputs?: Record<string, string | number>; plateContext?: PlateContext; embedded?: boolean; embeddedLocale?: "zh" | "en" }) {
  ...
  const defaults = useMemo(() => {
    ...
    if(calculatorId==='cfu'&&supplied.dnaUg===undefined)defaults.dnaUg=0;
    return defaults;
  }, [definition,initialInputs,calculatorId,plateContext]);
  const dialog=useModalDialog();
  const layout = workspaceLayout(calculatorId);
  const [cfuEfficiency,setCfuEfficiency]=useState(false);
  const [mobilePane, setMobilePane] = useState("inputs");
  const inputSection=useRef<HTMLDivElement>(null);
  const paneSwitch=useRef<HTMLElement>(null);
  const [initialDraft,setInitialDraft]=useState<CalculatorState["drafts"][string]|null>(null);
  ...
  function edit(next:Record<string,unknown>, nextExample=example,restoreUnits=false) {
    if(!restoreUnits&&calculatorId==='unit-converter'&&next.dimension!==inputs.dimension)next={...next,fromUnit:'',toUnit:''};
  ...
  function runCalculation(event?: FormEvent) {
    ...
    try {
      const next = calculate({ calculatorId, inputs:calculatorId==='cfu'&&!cfuEfficiency&&!(Number(inputs.dnaUg)>0)?{...inputs,dnaUg:0}:inputs });
      setResult(next);
      ...
      setError("");
      if(calculatorId==="master-mix"&&paneSwitch.current&&getComputedStyle(paneSwitch.current).display!=="none"){setMobilePane("result");requestAnimationFrame(()=>{resultSection.current?.focus({preventScroll:true});resultSection.current?.scrollIntoView({block:"start",behavior:"instant"});});}
  ...
  async function switchMixMode() {
    const next={...inputs};
    if(Array.isArray(next.groups)){
      const first=(next.groups as ReactionGroup[])[0];
      if(!first)return;
      if(!await dialog.confirm({title:zh?'切换为单组配液？':'Switch to a single group?',description:zh?`保留第一组“${first.name}”的组分及反应数；其他组将移除。`:`Keep the components and reaction count of the first group “${first.name}”; remove other groups.`,confirmLabel:zh?'切换':'Switch',cancelLabel:zh?'取消':'Cancel'}))return;
      delete next.groups;next.rows=first.rows;next.samples=first.reactions;next.replicates='1';next.controls='0';
    }else{
      const count=Number(inputs.samples)*Number(inputs.replicates)+Number(inputs.controls);
      next.groups=[{name:zh?'实验组1':'Group 1',reactions:Number.isInteger(count)&&count>0?String(count):'',rows:Array.isArray(inputs.rows)?inputs.rows:[]}];
    }
    edit(next);
  }

  function saveResult() {
    if (!state || !result || historical || example || !canCopyResult(result)) return;
  ...
  const presetControls=<CalculatorPresets zh={zh} presets={presets} disabled={!state||!!persistenceWarning} onSave={savePreset} onLoad={preset=>restore(preset.inputs,preset.methodVersion,preset.source==="synthetic example")} onRename={(id,name)=>{if(state)update(renamePreset(state,id,name));}} onDelete={id=>{if(state)update(deletePreset(state,id));}}/>;
  const loadControls=<div className="flex flex-wrap gap-3"><button type="button" className="min-h-11 text-sm text-moss" onClick={()=>edit({...defaults,...definition.exampleInputs},true)}>{zh?'载入示例':'Load example'}</button>{initialDraft?<button className="min-h-11 text-sm text-moss" type="button" onClick={()=>{const draft=initialDraft;if(draft)restore(draft.inputs,undefined,draft.example);}}>{zh?'恢复上次草稿':'Restore draft'}</button>:null}</div>;

  return (
    <div className="calculator-workbench calculator-compact calculator-stack" data-calculator={calculatorId} data-layout={layout} data-pane={mobilePane}>
      ...
      {layout !== 'simple' ? <nav ref={paneSwitch} className="calculator-pane-switch" aria-label={zh?'工作区':'Workspace'}>{[['inputs',zh?'配方 / 数据':'Inputs'],['result',zh?'结果':'Result']].map(([pane,label])=><button key={pane} type="button" aria-pressed={mobilePane===pane} onClick={()=>setMobilePane(pane)}>{label}</button>)}</nav>:null}
      <div ref={inputSection} className={`calculator-workspace-grid ${calculatorId==='transfection'?'transfection-workspace':''}`}>
        <Card className="calculator-input-panel">
          <CardHeader title={zh ? "输入" : "Inputs"} action={<div className="reaction-input-tools">{calculatorId==='master-mix'?<>{presetControls}{loadControls}</>:null}<button title={zh?"重置全部输入，保留已存预设与历史":"Reset all inputs; keep saved presets and history"} type="button" onClick={() => { edit(defaults,false); }} className="inline-flex items-center gap-1.5 text-xs font-medium text-muted hover:text-moss"><RotateCcw className="h-3.5 w-3.5" />{zh ? "重置" : "Reset"}</button></div>} />
          <CardBody>
            <form onSubmit={runCalculation} className="calculator-input-form">
              {calculatorId==='transfection'?<TransfectionEditor ref={transfectionEditor} zh={zh} plan={inputs.transfectionPlan as TransfectionPlan|undefined} onChange={transfectionPlan=>edit({...inputs,transfectionPlan,__displayUnits:{}})}/>:null}
              {calculatorId==='master-mix'?<button className="calculator-group-mode" type="button" onClick={()=>void switchMixMode()}>{Array.isArray(inputs.groups)?(zh?'多组配液 · 切换单组':'Multiple groups · Switch to single'):(zh?'单组配液 · 切换多组':'Single group · Switch to multiple')}</button>:null}
              <div className="calculator-fields">
                {orderedFields(definition.fields,layout).filter(field=>isFieldVisible(calculatorId,field.key,inputs)&&!(calculatorId==='cfu'&&field.key==='dnaUg'&&!cfuEfficiency&&!(Number(inputs.dnaUg)>0))&&!(calculatorId==='wb-loading'&&Array.isArray(inputs.samples)&&field.key==='sampleConcentrationUgUl')).map((field) => <div key={field.key} data-field-key={field.key} data-width={fieldWidth(field)} className={`calculator-field ${field.type==='textarea'?'calculator-field-wide':''} ${field.unit&&compatibleUnits(field.unit).length>1?"calculator-quantity":""}`}>{['points','standards','tcidSeries'].includes(field.key)&&field.type==='textarea'?<CurveEditor columnLabels={field.key==='tcidSeries'?(zh?['稀释度','阳性孔数','总孔数']:['Dilution','Positive wells','Total wells']):[`${zh?'浓度':'Concentration'} (${inputs.concentrationUnit|| (zh?'请选择单位':'choose unit')})`,calculatorId==='bradford-bca'?(zh?'吸光度':'Absorbance'):(zh?'反应值':'Response')]} zh={zh} value={String(inputs[field.key]??'')} onChange={value=>edit({...inputs,[field.key]:value})}/>:<CalculatorField field={{...field,...(calculatorId==='unit-converter'&&['fromUnit','toUnit'].includes(field.key)?{type:'select' as const,options:[{value:'',label:'Choose unit',labelZh:'选择单位'},...Object.keys(units).filter(unit=>{const dimension=units[unit].dimension;return inputs.dimension==='concentration'?['molar-concentration','mass-concentration'].includes(dimension):dimension===inputs.dimension;}).filter(unit=>field.key!=='toUnit'||!inputs.fromUnit||compatibleUnits(String(inputs.fromUnit)).includes(unit)).map(unit=>({value:unit,label:unit,labelZh:unit}))]}:{}),unit:field.unit&&compatibleUnits(field.unit).length>1?"":String(inputs[`${field.key}Unit`]??field.unit??"")}} zh={zh} value={inputs[field.key]??''} onChange={(value)=>edit({...inputs,[field.key]:value,...(calculatorId==='unit-converter'&&field.key==='fromUnit'&&!compatibleUnits(String(value)).includes(String(inputs.toUnit))?{toUnit:''}:{})})}/>} {field.unit&&compatibleUnits(field.unit).length>1?<select aria-label={`${zh?field.labelZh:field.label} ${zh?'单位':'unit'}`} className="min-h-11 min-w-0 max-w-[105px] rounded-lg border border-hairline px-2 text-sm" value={String(inputs[`${field.key}Unit`]??field.unit)} onChange={event=>{try { const previous=String(inputs[`${field.key}Unit`]??field.unit);const value=String(inputs[field.key]??'').trim();edit({...inputs,[field.key]:value?String(convert(parseScalar(value),previous,event.target.value)):'',[`${field.key}Unit`]:event.target.value});}catch{setError(zh?'请先完成数值输入，再切换单位。':'Complete the number before changing units.');}}}>{(['dilution','reagent-dosing','fold-dilution'].includes(calculatorId)&&['stockConcentration','targetConcentration','initialConcentration'].includes(field.key)?Object.keys(units).filter(unit=>['molar-concentration','mass-concentration'].includes(units[unit].dimension)):compatibleUnits(field.unit)).map(unit=><option key={unit}>{unit}</option>)}</select>:null}</div>)}
              </div>
              {calculatorId==='cfu'?<label className="text-xs"><input type="checkbox" checked={cfuEfficiency||Number(inputs.dnaUg)>0} onChange={e=>{setCfuEfficiency(e.target.checked);if(!e.target.checked)edit({...inputs,dnaUg:0});}}/>{zh?'计算转化效率':'Calculate transformation efficiency'}</label>:null}
              ...
              {error ? <p role="alert" tabIndex={-1} className="rounded-[var(--ln-radius-control-lg)] border border-danger/25 bg-danger-surface px-3 py-2 text-xs leading-5 text-danger">{error}</p> : null}
              <button type="submit" className="focus-ring inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-[var(--ln-radius-control-lg)] bg-moss px-4 text-sm font-semibold text-warm sm:w-auto"><Calculator className="h-4 w-4" />{zh ? "计算" : "Calculate"}</button></form>{calculatorId!=='master-mix'?presetControls:null}<div className="calculator-secondary-inputs">{layout!=='simple'?<button type="button" className="calculator-mobile-result" onClick={()=>setMobilePane('result')}>{zh?'结果':'Result'}</button>:null}
              {calculatorId!=='master-mix'?loadControls:null}
        ...
        <div ref={resultSection} tabIndex={-1} className="calculator-result-panel min-w-0 calculator-stack">
          <Card>
          ...
          {calculatorId!=='master-mix'?<details className="rounded-lg border border-hairline p-3"><summary className="min-h-11 cursor-pointer text-sm">{zh?'方法与假设':'Method and assumptions'}</summary><p className="text-xs leading-5 text-graphite">{zh?definition.methodZh:definition.method}</p><p className="mt-2 text-xs text-muted">{definition.methodVersion}</p></details>:null}
        </div>
      </div>
