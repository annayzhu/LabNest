...
export function CalculatorCatalog() {
  ...
  return (
    <div className="space-y-5">
        ...
        <Card>
          <CardBody className="p-4 sm:p-5">
            <div className="flex items-center gap-3">
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[10px] bg-sage-surface text-moss"><Calculator className="h-5 w-5" aria-hidden /></span>
            ...
            <label className="mt-4 flex h-10 items-center gap-2 rounded-[9px] border border-hairline bg-warm/45 px-3 focus-within:border-moss">
      ...
      {persistenceWarning ? <p role="status" className="rounded-[8px] border border-warning/25 bg-warning-surface px-3 py-2 text-xs text-warning">{zh ? "浏览器存储不可用或空间不足；本次更改仅保留在当前页面。" : persistenceWarning}</p> : null}
              ...
              {tools.map((tool) => {
                ...
                return (
                  <div key={tool.id} className="group grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 px-4 py-3 transition hover:bg-warm/45">
                    <Link href={`/tools/calculator/${tool.id}`} className="focus-ring min-w-0 rounded-[6px]">
                    ...
                    {state ? <button type="button" onClick={() => update(toggleFavorite(state, tool.id))} className="focus-ring flex h-8 w-8 items-center justify-center rounded-[7px] text-muted hover:bg-sage-surface hover:text-moss" aria-label={favorite ? (zh ? "取消固定" : "Unpin") : (zh ? "固定工具" : "Pin tool")}>{favorite ? <PinOff className="h-4 w-4" /> : <Pin className="h-4 w-4" />}</button> : null}
                  </div>
                );
              })}
            </CardBody>
...
export function CalculatorWorkbench({ calculatorId, initialInputs = {}, plateContext }: { calculatorId: string; initialInputs?: Record<string, string | number>; plateContext?: PlateContext }) {
  ...
  if (calculatorId === "colony-counter") {
    return <div className="space-y-4">{persistenceWarning ? <p role="status" className="rounded-[8px] border border-warning/25 bg-warning-surface px-3 py-2 text-xs text-warning">{zh ? "浏览器存储不可用或空间不足；本次更改仅保留在当前页面。" : persistenceWarning}</p> : null}<ColonyCounter definition={definition} zh={zh} state={state} update={update} /></div>;
  }
  ...
  return (
    <div className="space-y-4">
        ...
        <Card>
          ...
          <CardBody>
            <form onSubmit={runCalculation} className="space-y-4">
              ...
              {error ? <p role="alert" className="rounded-[8px] border border-danger/25 bg-danger-surface px-3 py-2 text-xs leading-5 text-danger">{error}</p> : null}
              <button type="submit" className="focus-ring inline-flex h-10 w-full items-center justify-center gap-2 rounded-[8px] bg-moss px-4 text-sm font-semibold text-warm sm:w-auto"><Calculator className="h-4 w-4" />{zh ? "计算" : "Calculate"}</button>
            </form>
          </CardBody>
        </Card>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card><CardHeader title={zh ? "预设" : "Presets"} /><CardBody className="space-y-3"><div className="flex gap-2"><input value={presetName} onChange={(event) => setPresetName(event.target.value)} placeholder={zh ? "预设名称" : "Preset name"} className="h-9 min-w-0 flex-1 rounded-[8px] border border-hairline bg-warm/30 px-3 text-sm outline-none focus:border-moss" /><button type="button" onClick={savePreset} disabled={!presetName.trim()} className="focus-ring inline-flex h-9 items-center gap-1.5 rounded-[8px] border border-hairline px-3 text-xs font-medium text-graphite disabled:opacity-40"><Save className="h-3.5 w-3.5" />{zh ? "保存" : "Save"}</button></div>{presets.length ? <div className="space-y-1">{presets.map((preset) => <div key={preset.id} className="flex items-center gap-1"><button type="button" onClick={() => { setInputs(preset.inputs); setResult(null); }} className="min-w-0 flex-1 truncate rounded-full bg-sage-surface px-3 py-1 text-left text-xs font-medium text-moss">{preset.name}</button><button type="button" onClick={() => state && update(deletePreset(state, preset.id))} className="flex h-7 w-7 items-center justify-center rounded-full text-muted hover:bg-danger-surface hover:text-danger" aria-label={zh ? "删除预设" : "Delete preset"}><X className="h-3.5 w-3.5" /></button></div>)}</div> : <p className="text-xs text-muted">{zh ? "尚无预设。" : "No presets yet."}</p>}</CardBody></Card>
        <Card><CardHeader title={zh ? "最近结果" : "Recent results"} /><CardBody className="space-y-2">{history.length ? history.map((item) => <button type="button" key={item.id} onClick={() => { setInputs(item.inputs); setResult({ calculatorId, methodVersion: item.methodVersion, outputs: item.outputs, outputMap: Object.fromEntries(item.outputs.map((output) => [output.key, output.value])), warnings: item.warnings, notes: [] }); }} className="flex w-full items-center justify-between gap-3 rounded-[7px] px-2 py-1.5 text-left text-xs hover:bg-warm"><span className="truncate text-graphite">{new Date(item.createdAt).toLocaleString(locale)}</span><History className="h-3.5 w-3.5 shrink-0 text-muted" /></button>) : <p className="text-xs text-muted">{zh ? "保存后的结果会显示在这里。" : "Saved results appear here."}</p>}</CardBody></Card>
      </div>
    </div>
  );
}
...
function FieldControl({ field, zh, value, onChange }: { field: CalculatorDefinition["fields"][number]; zh: boolean; value: unknown; onChange: (value: string | number) => void }) {
  ...
  const shared = "mt-1.5 w-full rounded-[8px] border border-hairline bg-warm/30 px-3 text-sm text-ink outline-none transition focus:border-moss focus:bg-surface";
...
function ColonyCounter({ definition, zh, state, update }: { definition: CalculatorDefinition; zh: boolean; state: CalculatorState | null; update: (state: CalculatorState) => void }) {
  ...
  return <div className="space-y-4"><div className="flex items-center gap-3"><Link href="/tools/calculator" className="focus-ring flex h-9 w-9 items-center justify-center rounded-[8px] border border-hairline text-muted"><ArrowLeft className="h-4 w-4" /></Link><div><h1 className="font-serif text-[21px] font-medium text-ink md:text-[24px]">{zh ? definition.nameZh : definition.name}</h1><p className="text-xs text-muted">{zh ? "照片仅在当前浏览器会话中参与计数，不进入结果、历史或导出。" : "The photo is used only in this browser session and never enters results, history, or exports."}</p></div></div><div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_300px]"><Card><CardHeader title={zh ? "图像与识别标记" : "Image & detection overlay"} /><CardBody><div className="flex min-h-[360px] items-center justify-center overflow-hidden rounded-[9px] border border-dashed border-hairline bg-warm/35">{previewUrl ? <canvas ref={canvasRef} onClick={handleCanvasClick} className="max-h-[68vh] max-w-full cursor-crosshair object-contain" /> : <label className="flex cursor-pointer flex-col items-center gap-3 px-6 py-12 text-center"><Upload className="h-7 w-7 text-muted" /><span className="text-sm font-medium text-graphite">{zh ? "上传平皿或噬菌斑照片" : "Upload a plate or plaque image"}</span><span className="text-xs leading-5 text-muted">{zh ? "JPG / PNG；图像不会保存" : "JPG / PNG; image is not persisted"}</span><input type="file" accept="image/*" capture="environment" className="sr-only" onChange={(event) => loadImage(event.target.files?.[0])} /></label>}</div>{previewUrl ? <div className="mt-3 flex flex-wrap gap-2"><label className="focus-ring inline-flex h-9 cursor-pointer items-center gap-2 rounded-[8px] border border-hairline px-3 text-xs font-medium"><Upload className="h-3.5 w-3.5" />{zh ? "更换照片" : "Replace image"}<input type="file" accept="image/*" capture="environment" className="sr-only" onChange={(event) => loadImage(event.target.files?.[0])} /></label><button type="button" onClick={detect} className="focus-ring h-9 rounded-[8px] bg-moss px-3 text-xs font-semibold text-warm">{zh ? "重新识别" : "Detect again"}</button></div> : null}</CardBody></Card><div className="space-y-4"><Card><CardHeader title={zh ? "识别设置" : "Detection settings"} /><CardBody className="space-y-4"><label className="block text-xs font-medium text-graphite">{zh ? "灰度阈值" : "Threshold"}<input type="range" min="20" max="235" value={threshold} onChange={(event) => { setThreshold(Number(event.target.value)); setConfirmed(false); }} className="mt-2 w-full accent-[var(--color-moss)]" /><span className="font-mono text-[10px] text-muted">{threshold}</span></label><label className="block text-xs font-medium text-graphite">{zh ? "最小区域" : "Minimum area"}<input type="number" min="2" value={minimumArea} onChange={(event) => { setMinimumArea(Number(event.target.value)); setConfirmed(false); }} className="mt-1.5 h-9 w-full rounded-[8px] border border-hairline bg-warm/30 px-3" /></label><div className="grid grid-cols-3 gap-1">{(["add", "remove", "exclude"] as const).map((item) => <button key={item} type="button" onClick={() => setMode(item)} className={`h-8 rounded-[7px] text-[11px] font-medium ${mode === item ? "bg-sage-surface text-moss" : "border border-hairline text-muted"}`}>{zh ? { add: "补加", remove: "扣除", exclude: "排除区" }[item] : item}</button>)}</div><p className="text-[11px] leading-5 text-muted">{zh ? "选择模式后点击图像：补加/扣除计数，或排除局部识别区域。" : "Choose a mode, then click the image to add/remove a count or exclude a local region."}</p></CardBody></Card><Card><CardHeader title={zh ? "计数结果" : "Count"} /><CardBody className="space-y-3"><div className="grid grid-cols-2 gap-2"><div className="rounded-[8px] bg-warm p-3"><p className="text-[10px] text-muted">{zh ? "自动识别" : "Detected"}</p><p className="mt-1 font-mono text-xl font-semibold text-ink">{spots.length}</p></div><div className="rounded-[8px] bg-sage-surface p-3"><p className="text-[10px] text-moss">{zh ? "复核后" : "Reviewed"}</p><p className="mt-1 font-mono text-xl font-semibold text-moss">{finalCount}</p></div></div><p className="text-xs text-muted">{zh ? "人工调整" : "Manual adjustment"}: {manualAdjustment > 0 ? "+" : ""}{manualAdjustment}</p><button type="button" disabled={!previewUrl} onClick={() => setConfirmed(true)} className="focus-ring inline-flex h-9 w-full items-center justify-center gap-2 rounded-[8px] bg-moss text-xs font-semibold text-warm disabled:opacity-40"><Check className="h-3.5 w-3.5" />{confirmed ? (zh ? "已确认" : "Confirmed") : (zh ? "确认计数" : "Confirm count")}</button><button type="button" disabled={!confirmed || !state} onClick={saveCount} className="focus-ring inline-flex h-9 w-full items-center justify-center gap-2 rounded-[8px] border border-moss text-xs font-semibold text-moss disabled:opacity-40"><Save className="h-3.5 w-3.5" />{zh ? "仅保存数字与参数" : "Save numbers & settings only"}</button></CardBody></Card></div></div></div>;
}
