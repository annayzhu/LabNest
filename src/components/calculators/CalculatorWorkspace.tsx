...
export function CalculatorWorkbench({ calculatorId, initialInputs = {}, plateContext, embedded = false, embeddedLocale }: { calculatorId: string; initialInputs?: Record<string, string | number>; plateContext?: PlateContext; embedded?: boolean; embeddedLocale?: "zh" | "en" }) {
  ...
  return (
    <div className="calculator-workbench calculator-compact calculator-stack" data-layout={layout} data-pane={mobilePane}>
      ...
      {layout !== 'simple' ? <nav className="calculator-pane-switch" aria-label={zh?'工作区':'Workspace'}>{[['inputs',zh?'配方 / 数据':'Inputs'],['result',zh?'结果':'Result']].map(([pane,label])=><button key={pane} type="button" aria-pressed={mobilePane===pane} onClick={()=>setMobilePane(pane)}>{label}</button>)}</nav>:null}
      <div className={`calculator-workspace-grid ${calculatorId==='transfection'?'transfection-workspace':''}`}>
        <Card className="calculator-input-panel">
          ...
          <CardBody>
            <form onSubmit={runCalculation} className="calculator-input-form">
              ...
              {calculatorId==='master-mix'?<button className="calculator-group-mode" type="button" onClick={()=>{const next={...inputs};if(Array.isArray(next.groups))delete next.groups;else next.groups=[{name:zh?'实验组1':'Group 1',reactions:'',rows:Array.isArray(inputs.rows)?inputs.rows:[]}];edit(next);}}>{Array.isArray(inputs.groups)?(zh?'多组配液 · 切换单组':'Multiple groups · Switch to single'):(zh?'单组配液 · 切换多组':'Single group · Switch to multiple')}</button>:null}
        ...
        <div ref={resultSection} className="calculator-result-panel min-w-0 calculator-stack">
          <Card>
