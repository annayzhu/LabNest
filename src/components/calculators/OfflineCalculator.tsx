...
export function OfflineCalculator({zh}:{zh:boolean}) {
 ...
 async function cachePage(){
  try {
   ...
   const registration=await navigator.serviceWorker.register('/tools/calculator/worker.js',{scope:'/tools/calculator/'});
 ...
 return <details className="text-xs text-muted"><summary className="min-h-11 cursor-pointer">{zh?'离线使用':'Offline use'}</summary><button type="button" className="min-h-11 text-moss" onClick={cachePage}>{zh?'缓存此计算页':'Cache this calculation page'}</button><p role="status">{status}</p></details>;
}
