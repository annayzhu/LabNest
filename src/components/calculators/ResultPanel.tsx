...
import {primaryOutputKeys} from '@/lib/calculators/workspace-presentation';
...
export function ResultPanel({result,zh,onSave,disabled,onApplyToPlate,onUnit}:{result:CalculatorResult;zh:boolean;onSave:()=>void;disabled:boolean;onApplyToPlate?:()=>void;onUnit?:(key:string,unit:string)=>void}) {
 ...
 const local=(value:unknown)=>{const known:Record<string,[string,string]>={'是 / Yes':['是','Yes'],'独立加样 / Separate':['单独加入','Separate'],'水 / Water':['水','Water'],'有效 / Valid':['有效','Valid'],'已备起始液 / Prepared starting solution':['已备起始液','Prepared starting solution']};return typeof value==='string'&&known[value]?known[value][zh?0:1]:value;};
 ...
 const values=(items:typeof outputs)=><dl className="calculator-result-values">{items.map(output=><div key={output.key}><dt>{zh?output.labelZh:output.label}</dt><dd><span>{typeof output.value==='number'?formatQuantity(output.value):output.value}</span>{output.unit?picker(output.key,result.outputs.find(o=>o.key===output.key)!.unit!,zh?output.labelZh:output.label):null}</dd></div>)}</dl>;
 return <div className="min-w-0 space-y-3" data-calculator-result>
  {values(curve?outputs.slice(0,2):table.length?outputs.slice(0,3):outputs)}
  ...
  {result.warnings.map(w=><p key={w} className="rounded bg-warning-surface p-2 text-xs text-warning">{w}</p>)}
  ...
  {curve||table.length&&outputs.length>3?<details><summary>{zh?'完整结果与拟合参数':'Full results and fit parameters'}</summary>{values(outputs)}{curve?tableView:<div className="calculator-data-scroll"><table><tbody>{presentedTable(result,zh).map((row,i)=><tr key={i}>{Object.values(row).map((v,j)=><td key={j}>{String(v)}</td>)}</tr>)}</tbody></table></div>}</details>:null}
  ...
  {result.notes.length?<details><summary>{zh?'结果说明':'Result notes'}</summary>{result.notes.map(note=><p key={note} className="text-xs text-muted">{note}</p>)}</details>:null}
 </div>;
}
