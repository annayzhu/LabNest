import {calculateTransfection,type TransfectionPlan} from './transfection';
...
function calculateInternal(request: CalculatorRequest): CalculatorResult {
  ...
  switch (definition.id) {
    ...
    case "transfection": { if(i.transfectionPlan!==undefined)return calculateTransfection(i.transfectionPlan as TransfectionPlan); const n = num(i, "wells", { positive: true }) * num(i, "replicates", { positive: true }); const f = 1 + num(i, "overagePercent", { min: 0 }) / 100; const dna = num(i, "dnaUgPerWell", { min: 0 }) * n * f; const reagent = dna * num(i, "reagentUlPerUg", { min: 0 }); const total = num(i, "complexVolumeUlPerWell", { positive: true }) * n * f; const dnaVolume = dna / num(i, "dnaConcentrationUgUl", { positive: true }); if (dnaVolume + reagent > total) throw new Error("核酸和试剂超过复合物体积 / DNA and reagent exceed complex volume");
      let tubeTable;
      ...
      return finish({...definition,methodVersion:"transfection-v2"}, [out("dnaUg", "DNA", "DNA", dna, "µg"), out("reagentUl", "Transfection reagent", "转染试剂", reagent, "µL"), out("dnaVolumeUl", "DNA solution", "DNA原液", dnaVolume, "µL"), out("diluentUl", "Diluent", "稀释液", total - reagent - dnaVolume, "µL"), out("totalComplexUl", "Total complex", "复合物总量", total, "µL")], [], ["试剂、稀释液及混合顺序遵循所用产品说明 / Follow product-specific reagent, diluent and mixing instructions"], tubeTable); }
