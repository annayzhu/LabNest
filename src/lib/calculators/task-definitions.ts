import {exampleTransfectionPlan} from './transfection';
...
export function enhanceDefinition(d: CalculatorDefinition): CalculatorDefinition {
  ...
  if(d.id==='transfection'){d.methodVersion='transfection-v3';d.method='DNA mass or size-weighted molar ratios; siRNA pmol from final culture volume; independent groups and explicit tube volumes';d.methodZh='DNA质量或按分子量折算的摩尔比；siRNA按最终培养体积计算pmol；独立实验组与明确单管体积';d.exampleInputs={transfectionPlan:exampleTransfectionPlan()};d.fields.unshift(select('complexMode','Mixing template','混合模板',[['combined','Combined mixture','单体系'],['two-tube','Two separate tubes','两管分别配制再混合']]));d.fields.push(n('tubeAVolumeUl','Tube A final volume per well','每孔A管总体积','µL'));}
...
export function isFieldVisible(id: string, key: string, inputs: Record<string,unknown>) {
  ...
  if(id==='transfection'&&inputs.transfectionPlan!==undefined)return false;
