...
function positive(value:unknown,label:string,zero=false){let n:number;try{n=parseScalar(value);}catch{throw Error(`${label}: 请输入有效数字 / Enter a valid number`);}if(!Number.isFinite(n)||(zero?n<0:n<=0))throw Error(`${label}: ${zero?'须≥0 / must be ≥0':'须>0 / must be >0'}`);return n;}
