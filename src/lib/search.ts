...
function searchableSources(): SearchSource[] {
  return [
    ...
    ...experiments.map((experiment) => ({
      ...
      keywords: [experiment.title, experiment.purpose, experiment.projectName, ...experiment.tags],
    })),
    ...
    ...inventoryItems.map((item) => ({
      ...
      title: item.name,
      subtitle: item.principalInvestigator ?? item.englishName ?? item.brand ?? item.lotNumber ?? item.location,
